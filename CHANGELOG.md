...
All notable changes to Barrel Inference are documented here. The format
...
- Bounded, interruptible, self-recovering decode. Every context
  ...
  `barrel_inference_nif:request_abort/1` sets an atomic flag the callback
...
- `generated => [token_id()]` in the `barrel_inference_done` Stats map: the
...
- `barrel_inference:reset_session/2` recovery primitive. Forcibly drops a
  ...
  callers on the reset seq receive `{barrel_inference_error, Ref, engine_reset}`.
- `n_seq_max` and `available_seqs` keys in `barrel_inference:model_info/1`.
...
- Vendored llama.cpp bumped from `b9119` to `b9222`. No Barrel Inference
...
- BEAM segfault in `barrel_inference:apply_chat_template/2` on rendered
  ...
  the downstream's 64 MiB body limit: `BARREL_INFERENCE_MAX_TOKEN_TEXT`
  4 MiB → 64 MiB, `BARREL_INFERENCE_MAX_TOKENS` 1 M → 16 M. The token output
...
- `barrel_inference:continue/3` for caller-asserted chat-template continuation
...
- `## Tool-call handling` section in the README describing what
  Barrel Inference exposes (per-model `tool_call_markers`, the
  `{tool_call_delta, _}` / `{barrel_inference_tool_call_end, _, Full}`
...
- README rewritten for sharper top-of-funnel: tighter "Why" list,
  ...
  long example.
- Architecture diagram corrected — `barrel_inference_cache_ramfile_srv`
  and `barrel_inference_cache_disk_srv` are operator-started standalone
  servers, not children of `barrel_inference_cache_sup`. Added
  `barrel_inference_registry` and `barrel_inference_inflight` which were missing.
...
- `tool_call_markers => #{start, end, payload_start (optional),
  payload_end (optional)}` on `barrel_inference:load_model/2` Config. Each
...
- Streaming wire on `infer/4` gains
  `{barrel_inference_token, Ref, {tool_call_delta, Bin}}` per chunk and a
  single `{barrel_inference_tool_call_end, Ref, Full :: binary()}` per
  ...
  re-buffering (#39).
- `barrel_inference:prefill_only/3` accepting `Opts` with `parent_key`.
...
- `session_id => term()` on `infer/4` Params and
  ...
  sticky_busy}`. Release with `barrel_inference:end_session/2` (#41).
...
- `step_result()` on `barrel_inference_model_backend` gains four variants
...
- `barrel_inference_model_stub` phase machine re-keyed from sampler ref onto
...
- `thinking_markers => #{start := binary(), end := binary()}` on
  `barrel_inference:load_model/2` Config. The backend tokenises both
...
- `thinking_budget_tokens => pos_integer()` on `infer/4` `Params`.
  ...
  before the scheduler synthesises `{barrel_inference_thinking_end, _, _}`
  and re-routes further model thinking tokens through the normal
  post-thinking pipeline (#37).

### Changed

- `thinking_signature/2` callback on `barrel_inference_model_backend`
  bumped to `/3` (third argument is the accumulated thinking
  bytes). Backwards-compatible: only `barrel_inference_model_stub`
  implemented the optional callback in 0.3 and the stub +
  scheduler + new `barrel_inference_model_llama` move in lockstep (#36).
...
- `stop_sequences :: [binary()]` on `infer/4` `Params` and
  ...
  trimmed from the streamed `{barrel_inference_token, _, _}` chunks and the
...
- `thinking => enabled | disabled` on `infer/4` `Params` (default
  ...
  streaming requests receive `{barrel_inference_token, Ref, {thinking_delta,
  Bin}}` fragments and a single `{barrel_inference_thinking_end, Ref, Sig}`
...
- `barrel_inference_model_backend` gains `{thinking_token, token_id()}` and
...
- `llama_batch_init`, `llama_batch_free`, and `llama_batch_get_one`
  are now routed through `barrel_inference_safe_batch_*` `noexcept` shims
...
- `barrel_inference:complete/2,3` and `barrel_inference_model:complete/2,3` now return
  ...
  ```erlang
  %% before
  {ok, Reply, _Tokens} = barrel_inference:complete(Model, Prompt).
  %% after
  {ok, #{reply := Reply, finish_key := FK}} =
      barrel_inference:complete(Model, Prompt).
  ```

- Streaming `{barrel_inference_done, Ref, Stats}` (`infer/4`) `Stats` map
...
- `barrel_inference_nif:step/2` is the new multi-sequence batched decode
...
- Per-context `per_seq[]` tracking (`last_logits_idx`, `next_pos`)
  with `BARREL_INFERENCE_N_SEQ_MAX_CAP = 256`. `kv_unpack` / `kv_seq_rm`
  refresh the per-seq position so subsequent `step` calls see
  correct state.
- `barrel_inference_model_backend` behaviour gains optional callbacks
...
- `barrel_inference:prefill_only/2` and `barrel_inference_model:prefill_only/2` decode
...
- New public ETS table `barrel_inference_model_obs`, owned by
  `barrel_inference_inflight`, written by each model gen_statem on every
  state transition and read lock-free from any process (including
  remote nodes via `erpc`).
- Four new accessors on `barrel_inference`:
...
- `barrel_inference_nif:load_model/2` now reads three additional keys from
  `model_opts`:
...
- `barrel_inference_nif:new_context/2` reads three more from `context_opts`:
...
- New types exported from `barrel_inference_model`: `completion_result/0`,
  `prefill_result/0`.
- `barrel_inference_model_t` now owns the `tensor_split` buffer; the vendored
  llama.cpp aliases the pointer rather than copying it, so its
  storage must outlive the model.
- `barrel_inference_model_stub` derives per-seq tokens from
...
- `barrel_inference:queue_depth/0` returns O(1) inflight count via an
  atomics counter parked in persistent_term, readable cross-node
  via `erpc`. Used by the upcoming `barrel_inference_cluster` load
  balancer (least_loaded, power_of_two strategies).
- `barrel_inference:list_cached_prefixes/2` returns the longest cached
  prefix length of a token list for a given model on this node,
  across all cache tiers. Used by the cluster cache-affinity
  router.
- `barrel_inference_nif:vram_info/0` walks every loaded ggml backend and
...
- `barrel_inference:list_models/0` map gains `model_id`, `quant_tag`,
  `loaded_at_monotonic`, and `vram_estimate_b` keys. Existing
  keys (`id`, `pid`, `status`, etc.) are unchanged.

#### Speculative decoding (#13)

- `barrel_inference:draft_tokens/3` synchronously generates up to `Max`
...
- `barrel_inference:verify/4` runs `PrefixTokens ++ Candidates` through
  ...
  `{error, busy}`.
- Token-id streaming: `barrel_inference_model:stream_emit` now also sends
  `{barrel_inference_token_id, Ref, Id}` on every produced token, in
  addition to the existing `{barrel_inference_token, Ref, Bin}`.
...
- Optional callbacks `extra_metadata/1` (vram-related model
  ...
  `barrel_inference_model_backend`. Backends that omit either get a
  graceful `{error, not_supported}` fallback (#13).
- Optional callback `seq_clear/1` on `barrel_inference_model_backend`.
...
- Cold-path prefill KV-state leak: `barrel_inference_model:enter_prefilling`
...
- `barrel_inference_inflight:register/2` and `unregister/1` switched to
...
- New `c_src/CMakeLists.txt` options:
  ...
  scoped to the `barrel_inference_nif` target; default OFF.
...
- Native Erlang/OTP wrapper around llama.cpp via a single
  dirty-scheduler NIF (`barrel_inference_nif`) covering model load, context
  construction, tokenisation, prefill, single-token decode, and
  KV pack/unpack.
- Models are identified by `binary()` on the public API.
  `barrel_inference:load_model/2`, `complete/2,3`, `unload/1`, `status/1`,
  `evict/1`, `shutdown/1` take `binary() | pid()`. Internal
  registration uses `{via, barrel_inference_registry, BinaryId}` so user-
  supplied ids cannot exhaust the atom table.
- `barrel_inference:list_models/0` returning `[model_info()]` and
  `barrel_inference:model_info/1` keyed on a model id.
- Public `barrel_inference:tokenize/2` and `barrel_inference:detokenize/2` keyed on a
  model id. The low-level `barrel_inference_nif:tokenize/3` and
  `barrel_inference_nif:detokenize/2` remain available.
- `barrel_inference:unload_model/1` as an alias for `barrel_inference:unload/1`
  matching the OpenAI/Ollama-style naming downstream HTTP servers
  use.
- `barrel_inference:infer/4` streaming inference. Returns `{ok, Ref}`;
  tokens are delivered to the caller as `{barrel_inference_token, Ref, _}`,
  `{barrel_inference_done, Ref, Stats}`, `{barrel_inference_error, Ref, Reason}`.
- `barrel_inference:cancel/1`. Idempotent and fire-and-forget; observed
  between tokens.
- `barrel_inference:apply_chat_template/2`. Renders a normalised chat
  request (`messages`, `system`, `tools`) through the model's
  GGUF chat template and tokenises. Backed by
  `llama_chat_apply_template`.
- `barrel_inference:embed/2`. Per-sequence pooled embedding via
...
- Sampler parameters: `complete/3` and `infer/4` honour
  ...
  (`barrel_inference_nif:configure_sampler/2`). Chain order:
...
- `barrel_inference:load_adapter/2`, `unload_adapter/2`,
  `set_adapter_scale/3`, `list_adapters/1`. Per-adapter sha256 +
  scale fold into the cache via
  `barrel_inference_cache_key:effective_fingerprint/2`, so rows produced
...
- Seq-aware NIFs (infrastructure for 0.2 multi-seq batching):
  ...
  `barrel_inference_sampler_t` resource owning a standalone
...
- `barrel_inference_registry` module: ETS-backed `via` callback for binary
  model ids.
- `barrel_inference_inflight` module: `Ref -> ModelPid` table so
  `cancel/1` routes to the right gen_statem.
- `barrel_inference_model_backend` optional callbacks:
...
- Token-exact KV cache with three independently-supervised tiers:
  ...
  I/O).
- Sole-writer arbitration through `barrel_inference_cache_meta_srv`; reads
...
- Multi-turn warmth via explicit `parent_key` resume and
  ...
  clients.
- `barrel_inference_scheduler` memory-pressure poller with pluggable
  ...
  server.
- `barrel_inference_cache_writer` dirty-IO writer pool with a leak-proof
...
- `barrel_inference_nif_tests:load_model_rejects_non_existent_path_test` is
...
- New stub-backed coverage: sampler params (`barrel_inference_sampler_tests`),
  LoRA adapters + cache identity (`barrel_inference_lora_tests`), FIFO
  queueing of concurrent infers (`barrel_inference_streaming_tests`).
...
- `ROADMAP.md`: what 0.1 doesn't do yet (multi-seq concurrent
  ...
  compression, cluster).
- README closes with a teaser for the upcoming `barrel_inference_cluster`
  application: a separate OTP project that coordinates a fleet of
  Barrel Inference nodes (request distribution, cross-node speculative
...
[Unreleased]: https://github.com/barrel-platform/barrel_inference/compare/v0.2.0...HEAD
[0.2.0]: https://github.com/barrel-platform/barrel_inference/compare/v0.1.2...v0.2.0
[0.1.2]: https://github.com/barrel-platform/barrel_inference/compare/v0.1.1...v0.1.2
[0.1.1]: https://github.com/barrel-platform/barrel_inference/compare/v0.1.0...v0.1.1
[0.1.0]: https://github.com/barrel-platform/barrel_inference/releases/tag/v0.1.0
