...
%%
-module(barrel_inference_cache_kvc_tests).
...
build_then_parse_round_trip_test() ->
    ...
    Key = key_for(Meta),
    {ok, Info, ParsedPayload} = barrel_inference_cache_kvc:parse(Bin, Key),
    ...
    ?assertEqual(<<"0.1.0">>, maps:get(barrel_inference_version, Info)).
...
build_with_empty_payload_round_trip_test() ->
    ...
    Bin = <<Prefix/binary, Payload/binary>>,
    Key = key_for(Meta),
    {ok, _Info, <<>>} = barrel_inference_cache_kvc:parse(Bin, Key).
...
build_with_large_payload_round_trip_test() ->
    ...
    Bin = <<Prefix/binary, Payload/binary>>,
    Key = key_for(Meta),
    {ok, _Info, ParsedPayload} = barrel_inference_cache_kvc:parse(Bin, Key),
...
build_with_empty_prompt_text_test() ->
    ...
    Bin = <<Prefix/binary, Payload/binary>>,
    Key = key_for(Meta),
    {ok, Info, _} = barrel_inference_cache_kvc:parse(Bin, Key),
...
header_payload_byte_count_test() ->
    ...
    {ok, Prefix} = barrel_inference_cache_kvc:build(base_meta(), Payload),
...
parse_rejects_bad_magic_test() ->
    ...
    ?assertMatch({error, _}, barrel_inference_cache_kvc:parse(Bad, Key)).
...
parse_rejects_bad_version_test() ->
    ...
    ?assertMatch(
        ...
        barrel_inference_cache_kvc:parse(Bad, Key)
    ).
...
parse_rejects_crc_mismatch_test() ->
    ...
    ?assertMatch(
        ...
        barrel_inference_cache_kvc:parse(Bin, Key)
    ).
...
parse_rejects_key_mismatch_test() ->
    ...
    ?assertMatch(
        ...
        barrel_inference_cache_kvc:parse(Bin, WrongKey)
    ).

parse_rejects_truncated_test() ->
    {ok, Prefix} = barrel_inference_cache_kvc:build(base_meta(), <<"x">>),
    Truncated = binary:part(Prefix, 0, 40),
    ?assertMatch({error, _}, barrel_inference_cache_kvc:parse_meta(Truncated)).
...
parse_rejects_trailing_bytes_test() ->
    ...
    ?assertMatch({error, trailing_bytes}, barrel_inference_cache_kvc:parse_meta(Bin)).
...
parse_meta_does_not_check_crc_test() ->
    ...
    {ok, _Info} = barrel_inference_cache_kvc:parse_meta(Bin).
