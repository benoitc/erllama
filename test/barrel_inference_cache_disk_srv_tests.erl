...
%%
-module(barrel_inference_cache_disk_srv_tests).
...
save_then_load_round_trip_test() ->
    with_disk(fun(_Dir) ->
        ...
        {ok, Key, _Header, Size} = barrel_inference_cache_disk_srv:save(test_disk, Meta, Payload),
        ?assert(Size > byte_size(Payload)),
        ?assertEqual(Key, key_for(Meta)),
        {ok, _Info, P} = barrel_inference_cache_disk_srv:load(test_disk, Key),
...
save_writes_only_one_file_test() ->
    with_disk(fun(Dir) ->
        ...
        {ok, _Key, _Header, _Size} =
            barrel_inference_cache_disk_srv:save(test_disk, Meta, <<"x">>),
...
load_unknown_key_returns_miss_test() ->
    with_disk(fun(_Dir) ->
        ...
        ?assertEqual(miss, barrel_inference_cache_disk_srv:load(test_disk, Bogus))
    end).
...
delete_removes_file_test() ->
    with_disk(fun(_Dir) ->
        Meta = base_meta([1, 2, 3]),
        {ok, Key, _, _} = barrel_inference_cache_disk_srv:save(test_disk, Meta, <<"x">>),
        ok = barrel_inference_cache_disk_srv:delete(test_disk, Key),
        ?assertEqual(miss, barrel_inference_cache_disk_srv:load(test_disk, Key))
    end).

delete_unknown_key_is_idempotent_test() ->
    with_disk(fun(_Dir) ->
        Bogus = crypto:hash(sha256, <<"nope">>),
        ?assertEqual(ok, barrel_inference_cache_disk_srv:delete(test_disk, Bogus))
    end).
...
load_detects_payload_tamper_test() ->
    with_disk(fun(Dir) ->
        Meta = base_meta([1, 2, 3]),
        {ok, Key, _, _} =
            barrel_inference_cache_disk_srv:save(test_disk, Meta, <<"original">>),
        ...
        ?assertMatch(
            ...
            barrel_inference_cache_disk_srv:load(test_disk, Key)
        ),
...
init_sweeps_tmp_files_test() ->
    ...
    try
        ...
        {ok, _} = barrel_inference_cache_meta_srv:start_link(),
        {ok, _} = barrel_inference_cache_ram:start_link(),
        {ok, _} = barrel_inference_cache_disk_srv:start_link(scan_disk, Dir),
        try
            {ok, Entries} = file:list_dir(Dir),
            ?assertEqual([], Entries)
        after
...
init_registers_existing_kvc_files_test() ->
    ...
    try
        ...
        {ok, _} = barrel_inference_cache_meta_srv:start_link(),
        {ok, _} = barrel_inference_cache_ram:start_link(),
        {ok, _} = barrel_inference_cache_disk_srv:start_link(scan2_disk, Dir),
        try
            ?assertMatch({ok, _Row}, barrel_inference_cache_meta_srv:lookup_exact(Key))
        after
...
init_drops_corrupt_kvc_files_test() ->
    ...
    try
        ...
        {ok, _} = barrel_inference_cache_meta_srv:start_link(),
        {ok, _} = barrel_inference_cache_ram:start_link(),
        {ok, _} = barrel_inference_cache_disk_srv:start_link(scan3_disk, Dir),
        try
            {ok, Entries} = file:list_dir(Dir),
            ?assertEqual([], Entries)
        after
...
eexist_with_valid_existing_file_adopts_test() ->
    with_disk(fun(Dir) ->
        ...
        Meta = base_meta([1, 2, 3]),
        Key = key_for(Meta),
        {ok, Prefix} = barrel_inference_cache_kvc:build(Meta, <<"first">>),
        Path = filename:join(Dir, bin_to_hex(Key) ++ ".kvc"),
        ...
        {ok, AdoptedKey, _Header, _Size} =
            barrel_inference_cache_disk_srv:save(test_disk, Meta, <<"second">>),
...
eexist_with_corrupt_existing_file_replaces_test() ->
    with_disk(fun(Dir) ->
        ...
        {ok, _Key, _Header, _Size} =
            barrel_inference_cache_disk_srv:save(test_disk, Meta, <<"good payload">>),
        {ok, _Info, P} = barrel_inference_cache_disk_srv:load(test_disk, Key),
...
make_tmp_dir() ->
    ...
    Dir = filename:join(
        Base,
        "barrel_inference_cache_disk_srv_tests_" ++
