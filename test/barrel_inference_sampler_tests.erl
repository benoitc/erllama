...
-module(barrel_inference_sampler_tests).
-include_lib("eunit/include/eunit.hrl").

with_app(Body) ->
    {ok, Started} = application:ensure_all_started(barrel_inference),
    Dir = make_tmp_dir(),
    {ok, _} = barrel_inference_cache_disk_srv:start_link(sampler_disk, Dir),
    try
        Body()
    after
...
make_tmp_dir() ->
    Base = filename:basedir(user_cache, "barrel_inference-sampler-tests"),
...
empty_opts_no_sampler_cfg_test() ->
    ...
    with_app(fun() ->
        ...
        try
            {ok, _} = barrel_inference:complete(Id, <<"hello world">>),
...
full_sampler_set_lands_on_backend_test() ->
    with_app(fun() ->
        ...
        try
            {ok, _} = barrel_inference:complete(Id, <<"hello">>, Opts),
            Cfg = get_stub_cfg(Id),
...
grammar_opt_lands_on_backend_test() ->
    with_app(fun() ->
        ...
        {ok, _} = barrel_inference:load_model(Id, minimal_config()),
        try
            {ok, _} = barrel_inference:complete(
...
infer_path_also_configures_sampler_test() ->
    ...
    with_app(fun() ->
        ...
        {ok, _} = barrel_inference:load_model(Id, minimal_config()),
        try
            {ok, Tokens} = barrel_inference:tokenize(Id, <<"a b c">>),
...
drain(Ref) ->
    receive
        {barrel_inference_token, Ref, _} -> drain(Ref);
        {barrel_inference_done, Ref, _} -> ok;
        {barrel_inference_error, Ref, R} -> ?assert({unexpected_error, R} =:= ok)
    after 5000 ->
...
get_stub_cfg(ModelId) ->
    ...
    case barrel_inference_model:get_last_sampler_cfg(ModelId) of
        undefined -> #{};
        Cfg -> Cfg
    end.
