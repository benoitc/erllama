...
%%
-module(barrel_inference_nif_tests).
...
crc32c_check_string_test() ->
    ?assertEqual(16#E3069283, barrel_inference_nif:crc32c(<<"123456789">>)).

%% Empty input: by definition the digest of zero bytes is 0.
crc32c_empty_test() ->
    ?assertEqual(0, barrel_inference_nif:crc32c(<<>>)).
...
crc32c_zero_byte_test() ->
    ?assertEqual(16#527D5351, barrel_inference_nif:crc32c(<<0>>)).

%% 32 zero bytes: reference 0x8A9136AA.
crc32c_zeros_32_test() ->
    Bytes = binary:copy(<<0>>, 32),
    ?assertEqual(16#8A9136AA, barrel_inference_nif:crc32c(Bytes)).

%% 32 0xFF bytes: reference 0x62A8AB43.
crc32c_ones_32_test() ->
    Bytes = binary:copy(<<16#FF>>, 32),
    ?assertEqual(16#62A8AB43, barrel_inference_nif:crc32c(Bytes)).
...
crc32c_accepts_iolist_test() ->
    ...
    ?assertEqual(
        barrel_inference_nif:crc32c(Flat),
        barrel_inference_nif:crc32c(IoList)
    ).
...
crc32c_split_input_test() ->
    ...
    ?assertEqual(
        barrel_inference_nif:crc32c(Whole),
        barrel_inference_nif:crc32c(Split)
    ).

crc32c_badarg_atom_test() ->
    ?assertError(badarg, barrel_inference_nif:crc32c(not_iodata)).

crc32c_badarg_integer_test() ->
    ?assertError(badarg, barrel_inference_nif:crc32c(42)).
...
kv_pack_rejects_non_resource_test() ->
    ?assertError(badarg, barrel_inference_nif:kv_pack(make_ref(), [1, 2, 3], 3)).

kv_unpack_rejects_non_resource_test() ->
    ?assertError(badarg, barrel_inference_nif:kv_unpack(make_ref(), <<>>, 0)).

request_abort_rejects_non_resource_test() ->
    ?assertError(badarg, barrel_inference_nif:request_abort(make_ref())).
...
load_model_rejects_non_existent_path_test_() ->
    {timeout, 60, fun() ->
        Result = barrel_inference_nif:load_model(<<"/no/such/file.gguf">>, #{}),
...
nif_step_decode_without_prefill_rejects_test_() ->
    case os:getenv("LLAMA_TEST_MODEL") of
        ...
        Path ->
            {timeout, 60, fun() ->
                {ok, Model} =
                    barrel_inference_nif:load_model(list_to_binary(Path), #{n_gpu_layers => 0}),
                {ok, Ctx} =
                    barrel_inference_nif:new_context(Model, #{n_ctx => 256, n_seq_max => 2}),
                {ok, Sampler} = barrel_inference_nif:sampler_new(Ctx, #{}),
                try
                    ?assertEqual(
                        {error, no_logits},
                        barrel_inference_nif:step(Ctx, [{0, {decode, Sampler}}])
                    )
                after
                    ok = barrel_inference_nif:sampler_free(Sampler),
                    ok = barrel_inference_nif:free_context(Ctx),
                    ok = barrel_inference_nif:free_model(Model)
                end
            end}
    end.
    ...
    case os:getenv("LLAMA_TEST_MODEL") of
        ...
        Path ->
            {timeout, 60, fun() ->
                ...
                {ok, Sampler} = barrel_inference_nif:sampler_new(Ctx, #{}),
                try
                    Tokens = barrel_inference_nif:tokenize(
                        Model,
                        <<"Hello world">>,
                        #{add_special => true, parse_special => false}
                    ),
                    {ok, [{0, prefilled}]} =
                        barrel_inference_nif:step(Ctx, [{0, {prefill, Tokens}}]),
                    {ok, [{0, {token, T, _Eog}}]} =
                        barrel_inference_nif:step(Ctx, [{0, {decode, Sampler}}]),
                    ?assert(is_integer(T)),
                    ?assert(T >= 0)
    ...
    case os:getenv("LLAMA_TEST_MODEL") of
        ...
        Path ->
            {timeout, 60, fun() ->
                ...
                {ok, Ctx} =
                    barrel_inference_nif:new_context(Model, #{n_ctx => 256, n_seq_max => 2}),
                {ok, Sampler0} = barrel_inference_nif:sampler_new(Ctx, #{}),
                try
                    Tokens = barrel_inference_nif:tokenize(
                        Model,
                        <<"Hello world">>,
                    ...
                    {ok, [{0, prefilled}]} =
                        barrel_inference_nif:step(Ctx, [{0, {prefill, Tokens}}]),
                    {ok, Results} = barrel_inference_nif:step(Ctx, [
                        {0, {decode, Sampler0}},
                        {1, {prefill, Tokens}}
                    ]),
                ...
                after
                    ok = barrel_inference_nif:sampler_free(Sampler0),
                    ok = barrel_inference_nif:free_context(Ctx),
                    ok = barrel_inference_nif:free_model(Model)
                end
            end}
        ...
        Path ->
            {timeout, 60, fun() ->
                {ok, Model} =
                    barrel_inference_nif:load_model(list_to_binary(Path), #{n_gpu_layers => 0}),
                {ok, Ctx} =
                    barrel_inference_nif:new_context(Model, #{n_ctx => 64, n_seq_max => 2}),
                try
                    ?assertError(
                        badarg,
                        barrel_inference_nif:step(Ctx, [{99999, {prefill, [1, 2, 3]}}])
                    )
                after
                    ok = barrel_inference_nif:free_context(Ctx),
                    ok = barrel_inference_nif:free_model(Model)
        ...
        Path ->
            {timeout, 60, fun() ->
                ...
                try
                    ?assertError(
                        badarg,
                        barrel_inference_nif:new_context(Model, #{n_seq_max => 1000000})
                    )
                after
                    ok = barrel_inference_nif:free_model(Model)
                end
...
load_model_rejects_bad_split_mode_test_() ->
    {timeout, 60, fun() ->
        ?assertError(
            badarg,
            barrel_inference_nif:load_model(
        ...
        Path ->
            {timeout, 60, fun() ->
                ...
                try
                    ?assertError(
                        badarg,
                        barrel_inference_nif:new_context(Model, #{flash_attn => bogus})
                    )
                after
                    ok = barrel_inference_nif:free_model(Model)
                end
        ...
        Path ->
            {timeout, 60, fun() ->
                ...
                try
                    ?assertError(
                        badarg,
                        barrel_inference_nif:new_context(Model, #{type_k => zz})
                    )
                after
                    ok = barrel_inference_nif:free_model(Model)
                end
    ...
    case os:getenv("LLAMA_TEST_MODEL") of
        ...
        Path ->
            {timeout, 60, fun() ->
                ...
                try
                    {ok, Ctx1} =
                        barrel_inference_nif:new_context(Model, #{
                            n_ctx => 256, flash_attn => auto
                        }),
                    ok = barrel_inference_nif:free_context(Ctx1),
                    {ok, Ctx2} =
                        barrel_inference_nif:new_context(Model, #{
                            n_ctx => 256, flash_attn => false
                        }),
                    ok = barrel_inference_nif:free_context(Ctx2)
                after
                    ok = barrel_inference_nif:free_model(Model)
                end
            end}
...
vram_info_returns_ok_or_no_gpu_test_() ->
    {timeout, 60, fun() ->
        case barrel_inference_nif:vram_info() of
            {ok, M} ->
                ?assert(is_map(M)),
...
live_smoke(Path) ->
    {ok, Model} = barrel_inference_nif:load_model(Path, #{n_gpu_layers => 0}),
    Tokens = barrel_inference_nif:tokenize(Model, <<"Hello world">>, #{
        add_special => true, parse_special => false
    }),
    ?assert(is_list(Tokens)),
    ?assert(length(Tokens) > 0),
    {ok, Ctx} = barrel_inference_nif:new_context(Model, #{n_ctx => 256}),
    Bin = barrel_inference_nif:kv_pack(Ctx, Tokens, length(Tokens)),
    ...
    case os:getenv("LLAMA_TEST_MODEL") of
        ...
        Path ->
            {timeout, 60, fun() ->
                ...
                %% NIF's BARREL_INFERENCE_MAX_TOKENS = 1M cap.
                prefill_overflow(
                    list_to_binary(Path),
                    #{n_ctx => 64, n_batch => 64},
                    200_000,
        ...
        _Path ->
            {timeout, 300, fun() ->
                true = proper:quickcheck(
                    prop_barrel_inference_nif:prop_prefill_never_crashes(),
...
prefill_overflow(Path, CtxOpts, NTokens, ExpectedAtom) ->
    {ok, Model} = barrel_inference_nif:load_model(Path, #{n_gpu_layers => 0}),
    {ok, Ctx} = barrel_inference_nif:new_context(Model, CtxOpts),
    try
        ...
        ?assertEqual(
            {error, ExpectedAtom},
            barrel_inference_nif:prefill(Ctx, lists:duplicate(NTokens, 0))
        )
    after
        ok = barrel_inference_nif:free_context(Ctx),
        ok = barrel_inference_nif:free_model(Model)
...
embed_overflow(Path, CtxOpts, NTokens, ExpectedAtom) ->
    ...
    try
        ?assertEqual(
            {error, ExpectedAtom},
            barrel_inference_nif:embed(Ctx, lists:duplicate(NTokens, 0))
        )
    after
        ok = barrel_inference_nif:free_context(Ctx),
        ok = barrel_inference_nif:free_model(Model)
...
bad_content(Path) ->
    ...
    try
        ?assertEqual(
            ...
            barrel_inference_nif:apply_chat_template(Model, Req)
        )
    after
        ok = barrel_inference_nif:free_model(Model)
    end.
