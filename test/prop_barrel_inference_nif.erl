...
-module(prop_barrel_inference_nif).
            ...
            ?SETUP(
                ...
                ?FORALL(
                    ...
                    begin
                        ...
                        case barrel_inference_nif:prefill(Ctx, Tokens) of
                            ok -> true;
                            {error, _} -> true;
                            _Other -> false
                        end
            ...
            ?SETUP(
                ...
                ?FORALL(
                    ...
                    begin
                        ...
                        ok = barrel_inference_nif:kv_seq_rm(Ctx, 0, 0, -1),
                        ok = barrel_inference_nif:kv_seq_rm(Ctx, 1, 0, -1),
                        Tokens = lists:duplicate(N, 1),
                        ok = barrel_inference_nif:prefill(Ctx, Tokens),
                        Bin = barrel_inference_nif:kv_pack(Ctx, Tokens, length(Tokens), 0),
                        is_binary(Bin) andalso
                            barrel_inference_nif:kv_unpack(Ctx, Bin, 1) =:= ok
                    end
                )
            )
    end.
...
setup_resources(Path) ->
    ...
    {ok, Ctx} = barrel_inference_nif:new_context(
        Model, #{n_ctx => n_ctx(), n_batch => n_batch(), n_seq_max => 2}
    ),
    persistent_term:put({?MODULE, model}, Model),
    persistent_term:put({?MODULE, ctx}, Ctx),
    fun() ->
        _ = barrel_inference_nif:free_context(persistent_term:get({?MODULE, ctx})),
        _ = barrel_inference_nif:free_model(persistent_term:get({?MODULE, model})),
