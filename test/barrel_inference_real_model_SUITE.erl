...
%%       --suite=barrel_inference_real_model_SUITE
%% ```
%%
%% What it covers:
...
%%   prefix
%% @end
-module(barrel_inference_real_model_SUITE).
...
%% Mirror of the C-side BARREL_INFERENCE_MAX_TOKENS define. Kept in sync with
%% `c_src/barrel_inference_nif.c`; if you change one, change the other.
...
end_per_suite(_Config) ->
    application:stop(barrel_inference),
    ok.
...
init_per_testcase(TC, Config) ->
    ...
    {ok, _} = barrel_inference_cache_disk_srv:start_link(DiskSrv, Dir),
    Model = iolist_to_binary(["real_model_", atom_to_binary(TC)]),
    {ok, _} = barrel_inference_model:start_link(Model, model_config(Path, DiskSrv)),
    barrel_inference_cache_counters:reset(),
    [{disk_srv, DiskSrv}, {model, Model}, {dir, Dir} | Config].

end_per_testcase(_TC, Config) ->
    catch barrel_inference_model:stop(?config(model, Config)),
...
load_unload(Config) ->
    ...
    %% barrel_inference_registry's via callback, so erlang:whereis/1 (atom-only)
    %% is the wrong lookup.
    Model = ?config(model, Config),
    ?assert(is_pid(barrel_inference_registry:whereis_name(Model))),
    ok.

tokenize_decode_one(Config) ->
    Model = ?config(model, Config),
    {ok, #{reply := Reply}} = barrel_inference_model:complete(
...
pack_unpack_round_trip(Config) ->
    ...
    Model = ?config(model, Config),
    {ok, #{reply := Reply1}} = barrel_inference_model:complete(
        Model, ?LONG_PROMPT, #{response_tokens => 8, seed => 42}
    ),
    timer:sleep(300),
    Before = barrel_inference_cache:get_counters(),
    {ok, #{reply := Reply2}} = barrel_inference_model:complete(
        Model, ?LONG_PROMPT, #{response_tokens => 8, seed => 42}
    ),
...
cold_then_warm_complete(Config) ->
    ...
    timer:sleep(300),
    Mid = barrel_inference_cache:get_counters(),
...
warm_faster_than_cold(Config) ->
    ...
    %% Cold path.
    {Cold, _} = timer:tc(
        barrel_inference_model, complete, [Model, ?LONG_PROMPT, #{response_tokens => 1}]
    ),
    timer:sleep(500),
    %% Warm path.
    {Warm, _} = timer:tc(
...
longest_prefix_resume_without_parent_key(Config) ->
    ...
    {ok, _} = barrel_inference_model:complete(Model, Prompt1, #{response_tokens => 4}),
    timer:sleep(400),
    Before = barrel_inference_cache:get_counters(),
    {ok, _} = barrel_inference_model:complete(Model, Prompt2, #{response_tokens => 2}),
    After = barrel_inference_cache:get_counters(),
...
apply_chat_template_renders(Config) ->
    ...
    case barrel_inference:apply_chat_template(Model, Request) of
        {ok, Tokens} ->
...
apply_chat_template_includes_system(Config) ->
    ...
    case
        {
            barrel_inference:apply_chat_template(Model, Without),
            barrel_inference:apply_chat_template(Model, With)
        }
    of
        {{ok, A}, {ok, B}} ->
...
set_grammar_constrains_output(Config) ->
    Model = ?config(model, Config),
    {ok, [Pid]} = {ok, [barrel_inference_registry:whereis_name(Model)]},
    [_] = [Pid || is_pid(Pid)],
    {ok, PromptTokens} = barrel_inference:tokenize(Model, <<"Answer with yes or no:">>),
    Grammar = <<"root ::= \"yes\" | \"no\"">>,
    Params = #{response_tokens => 6, grammar => Grammar},
    {ok, Ref} = barrel_inference:infer(Model, PromptTokens, Params, self()),
...
clear_sampler_resets_to_greedy(Config) ->
    Model = ?config(model, Config),
    {ok, PromptTokens} = barrel_inference:tokenize(Model, <<"Hello.">>),
...
drain(Ref, TimeoutMs, Acc) ->
    receive
        {barrel_inference_token, Ref, B} -> drain(Ref, TimeoutMs, [B | Acc]);
        {barrel_inference_done, Ref, S} -> {lists:reverse(Acc), S};
        {barrel_inference_error, Ref, R} -> {error, R}
    after TimeoutMs ->
        timeout
    end.
...
seed_determinism(Config) ->
    Model = ?config(model, Config),
    {ok, Tokens} = barrel_inference:tokenize(Model, ?SHORT_PROMPT),
...
grammar_plus_sampler(Config) ->
    Model = ?config(model, Config),
    {ok, Tokens} = barrel_inference:tokenize(Model, <<"Answer with yes or no:">>),
    Params = #{
        response_tokens => 6,
        grammar => <<"root ::= \"yes\" | \"no\"">>,
        temperature => 0.7,
...
chunked_prefill_matches_unchunked(Config) ->
    ...
    try
        {ok, Tokens} = barrel_inference:tokenize(ModelA, ?LONG_PROMPT),
        {TextA, _} = run_infer(ModelA, Tokens, Params),
        {TextB, _} = run_infer(ModelB, Tokens, Params),
        ?assertEqual(TextA, TextB)
    after
        catch barrel_inference_model:stop(ModelA),
        catch barrel_inference_model:stop(ModelB),
...
start_chunked_model(Path, BaseDiskSrv, ChunkSize, Suffix) ->
    ...
    Dir = filename:join(
        filename:dirname(code:priv_dir(barrel_inference)),
        "chunked_prefill_" ++ binary_to_list(Suffix)
    ),
    ok = filelib:ensure_path(Dir),
    {ok, _} = barrel_inference_cache_disk_srv:start_link(Srv, Dir),
    ...
    {ok, _} = barrel_inference_model:start_link(Model, Base#{policy => Policy}),
    {Model, Srv}.

run_infer(Model, Tokens, Params) ->
    {ok, Ref} = barrel_inference:infer(Model, Tokens, Params, self()),
...
verify_accepted_count_le_k(Config) ->
    ...
    [
        begin
            ok = barrel_inference_nif:kv_seq_rm(Ctx, 0, length(Prompt), -1),
            ok = barrel_inference_nif:prefill(Ctx, Prompt),
            {ok, Accepted, _Next} = barrel_inference:verify(Model, Prompt, Cands, K),
...
continue_multi_turn_cache_delta(Config) ->
    ...
    Model = ?config(model, Config),
    SessionId = make_ref(),
    {ok, Prompt1} = barrel_inference:tokenize(Model, ?LONG_PROMPT),
    Resp = 4,
    ...
    {ok, Suffix2} = barrel_inference:tokenize(Model, <<" Tell me more.">>),
    {ok, Ref2} = barrel_inference:continue(
        Model,
        Suffix2,
        #{
    ...
    {ok, Suffix3} = barrel_inference:tokenize(Model, <<" And then?">>),
    {ok, Ref3} = barrel_inference:continue(
        Model,
        Suffix3,
        #{
            session_id => SessionId,
    ...
    ok = barrel_inference:end_session(Model, SessionId),
    ok.
...
%% The engine must accept inputs up to BARREL_INFERENCE_MAX_TOKEN_TEXT without
%% crashing and produce token outputs at or below BARREL_INFERENCE_MAX_TOKENS.
...
apply_chat_template_large_input(Config) ->
    ...
    case barrel_inference:apply_chat_template(Model, Probe) of
        {error, no_template} ->
            {skip, "model has no chat template in GGUF metadata"};
        {ok, _} ->
...
%% Engine contract: for inputs within BARREL_INFERENCE_MAX_TOKEN_TEXT, the
%% engine returns either {ok, Tokens} with length(Tokens) bounded by
%% BARREL_INFERENCE_MAX_TOKENS, or a clean {error, too_large} when the
...
assert_tokenize_safe(Model, Sz) ->
    Prompt = big_text(Sz),
    case barrel_inference:tokenize(Model, Prompt) of
        {ok, Tokens} ->
            ?assert(is_list(Tokens)),
            ?assert(length(Tokens) > 0),
            ?assert(length(Tokens) =< ?BARREL_INFERENCE_MAX_TOKENS),
            ct:log(
...
tokenize_over_text_cap(Config) ->
    %% Just above BARREL_INFERENCE_MAX_TOKEN_TEXT must produce a clean
    %% {error, too_large} rather than a BEAM crash. Synthetic input
    %% is exactly cap+1 bytes so we exercise the boundary check.
    Model = ?config(model, Config),
    Over = big_text(?BARREL_INFERENCE_MAX_TOKEN_TEXT + 1),
    ?assertEqual({error, too_large}, barrel_inference:tokenize(Model, Over)),
