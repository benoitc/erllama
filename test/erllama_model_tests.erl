...
ladder_fires_multiple_cold_saves_test() ->
    ...
    with_model(Policy, fun(Cfg) ->
        Before = maps:get(saves_cold, erllama_cache:get_counters()),
        {ok, _} = erllama_model:complete(<<"test_model">>, Prompt, #{
            response_tokens => 2
        }),
        timer:sleep(100),
        After = maps:get(saves_cold, erllama_cache:get_counters()),
...
byte_prefix_resumes_across_retokenisation_test() ->
    ...
    with_model(#{}, fun(Cfg) ->
        ...
        {ok, #{finish_key := StoredKey, cache_hit_kind := cold}} =
            erllama_model:prefill_only(<<"test_model">>, Stored),
        ...
        {ok, #{cache_hit_kind := Kind, cache_delta := #{read := Read}}} =
            erllama_model:prefill_only(<<"test_model">>, Turn2),
...
agent_prefix_checkpoint_fires_pinned_and_survives_evict_test() ->
    with_model(#{}, fun(Cfg) ->
        ...
        ?assert(maps:get(saves_agent_prefix, erllama_cache:get_counters()) >= 1),
        %% The agent_prefix row is pinned (despite the async save / pin race).
        {ok, Row} = erllama_cache_meta_srv:dump(AgentKey),
        ...
        ?assertMatch({ok, _}, erllama_cache_meta_srv:lookup_exact(AgentKey)),
        %% A later turn whose head matches resumes from the surviving
        %% pinned row (longest-byte-prefix), not cold.
        Before = maps:get(hits_longest_prefix, erllama_cache:get_counters()),
        {ok, _} = erllama_model:complete(
            <<"test_model">>, long_prompt(), #{response_tokens => 2}
        ),
        After = maps:get(hits_longest_prefix, erllama_cache:get_counters()),
...
build_cold_ladder_single_segment_injects_boundary_test() ->
    ...
    {First, Tail} = erllama_model:build_cold_ladder(Tokens, [Tokens], 3),
...
build_cold_ladder_tags_only_N_agent_prefix_test() ->
    ...
    {First, Tail} = erllama_model:build_cold_ladder(Tokens, Plain, 3),
    ?assertEqual(Tokens, First ++ ladder_seg_parts(Tail)),
    ?assertEqual(lists:seq(1, 3), First),
...
build_cold_ladder_no_boundary_all_cold_test() ->
    Tokens = lists:seq(1, 10),
    Plain = [lists:seq(1, 5), lists:seq(6, 10), []],
    {First, Tail} = erllama_model:build_cold_ladder(Tokens, Plain, undefined),
    ?assertEqual(Tokens, First ++ ladder_seg_parts(Tail)),
...
no_agent_prefix_without_boundary_test() ->
    with_model(#{}, fun(_Cfg) ->
        ...
        timer:sleep(100),
        ?assertEqual(0, maps:get(saves_agent_prefix, erllama_cache:get_counters()))
    end).
...
complete_finish_key_undefined_when_below_min_tokens_test() ->
    ...
    with_model(#{min_tokens => 10_000}, fun(_Cfg) ->
        {ok, #{finish_key := FinishKey}} =
            erllama_model:complete(<<"test_model">>, short_prompt(), #{
                response_tokens => 2
            }),
...
reclaim_lru_idle_pin_admits_new_session_test_() ->
    {timeout, 30, fun() ->
        with_model(#{}, #{context_opts => #{n_seq_max => 2, n_batch => 64}}, fun(_Cfg) ->
            ...
            ?assertEqual({ok, not_found}, erllama:reset_session(<<"test_model">>, A)),
            ?assertEqual({ok, recovered}, erllama:reset_session(<<"test_model">>, B))
        end)
    end}.
...
reclaim_skips_in_flight_seqs_test_() ->
    {timeout, 30, fun() ->
        with_model(#{}, #{context_opts => #{n_seq_max => 2, n_batch => 64}}, fun(_Cfg) ->
            ...
            ok = erllama_model:cancel(R1),
            ok = erllama_model:cancel(R2),
...
decode_failed_recovers_in_place_test_() ->
    {timeout, 30, fun() ->
        with_model(#{}, fun(_Cfg) ->
            Pid = erllama_registry:whereis_name(<<"test_model">>),
            ok = erllama_model_stub:wedge_next_step({decode_failed, 1}),
            {ok, Ref1} = erllama_model:infer(
                <<"test_model">>,
                prompt_tokens(long_prompt()),
                #{response_tokens => 4},
