...
%%
-module(barrel_inference_cache_policy_tests).
...
trim_boundary_too_short_returns_skip_test() ->
    ...
    ?assertEqual(
        {skip, too_short},
        barrel_inference_cache_policy:trim_boundary(lists:seq(1, 600), 32, 2048)
    ).

trim_boundary_at_align_threshold_test() ->
    %% Len=2080, Trim=32 -> AfterTrim=2048, Align=2048
    Tokens = lists:seq(1, 2080),
    {ok, Prefix} = barrel_inference_cache_policy:trim_boundary(Tokens, 32, 2048),
...
trim_boundary_zero_trim_test() ->
    ...
    {ok, Prefix} = barrel_inference_cache_policy:trim_boundary(Tokens, 0, 2048),
    ?assertEqual(4096, length(Prefix)).

trim_boundary_align_one_is_identity_test() ->
    Tokens = lists:seq(1, 100),
    %% Trim=0, Align=1 -> always full length
    {ok, Prefix} = barrel_inference_cache_policy:trim_boundary(Tokens, 0, 1),
...
trim_boundary_trim_exceeds_length_test() ->
    ...
    ?assertEqual(
        {skip, too_short},
        barrel_inference_cache_policy:trim_boundary(lists:seq(1, 10), 100, 2048)
    ).

trim_boundary_empty_input_test() ->
    ?assertEqual(
        {skip, too_short},
        barrel_inference_cache_policy:trim_boundary([], 0, 1)
    ).

trim_boundary_badarg_negative_trim_test() ->
    ?assertError(function_clause, barrel_inference_cache_policy:trim_boundary([1, 2, 3], -1, 1)).

trim_boundary_badarg_zero_align_test() ->
    ?assertError(function_clause, barrel_inference_cache_policy:trim_boundary([1, 2, 3], 0, 0)).
...
cold_save_split_below_min_test() ->
    ...
    ?assertEqual(no_save, barrel_inference_cache_policy:cold_save_split(Tokens, cfg())).
...
cold_save_split_returns_prefix_and_rest_test() ->
    Tokens = lists:seq(1, 4500),
    {trim, Prefix, Rest} = barrel_inference_cache_policy:cold_save_split(Tokens, cfg()),
    ?assertEqual(4096, length(Prefix)),
...
should_continued_save_below_interval_test() ->
    ...
    ?assertNot(barrel_inference_cache_policy:should_continued_save(3048, 2048, cfg())).

should_continued_save_at_interval_test() ->
    ?assert(barrel_inference_cache_policy:should_continued_save(4096, 2048, cfg())).

should_continued_save_below_min_total_test() ->
    %% Even though delta >= interval, total live count is below min_tokens
    ?assertNot(barrel_inference_cache_policy:should_continued_save(400, 0, cfg())).
...
should_finish_save_below_min_test() ->
    ?assertNot(barrel_inference_cache_policy:should_finish_save(400, cfg())).

should_finish_save_at_min_test() ->
    ?assert(barrel_inference_cache_policy:should_finish_save(512, cfg())).

should_finish_save_above_min_test() ->
    ?assert(barrel_inference_cache_policy:should_finish_save(8192, cfg())).
...
validate_config_ok_test() ->
    ?assertEqual(ok, barrel_inference_cache_policy:validate_config(cfg())).

validate_config_missing_key_test() ->
    Bad = maps:remove(continued_interval, cfg()),
    ?assertMatch(
        {error, {missing_keys, [continued_interval]}},
        barrel_inference_cache_policy:validate_config(Bad)
    ).
