...
%%
%% Boots the full barrel_inference application, runs realistic scenarios, and
...
init_per_suite(Config) ->
    {ok, _Apps} = application:ensure_all_started(barrel_inference),
    Config.
...
init_per_testcase(TC, Config) ->
    ...
    {ok, _} = barrel_inference_model:start_link(Model, model_config(DiskSrv)),
...
cold_then_warm_counters(Config) ->
    ...
    {ok, _} = barrel_inference_model:complete(Model, Prompt, #{response_tokens => 4}),
...
multi_turn_session_resume(Config) ->
    ...
    {ok, _Pub} = barrel_inference_cache_writer:save(
        DiskSrv,
        disk,
        BuildMeta,
        barrel_inference_cache_key:encode_tokens(PrefixTokens)
    ),
    %% Wait until visible.
    wait_until(
        fun() -> barrel_inference_cache_meta_srv:lookup_exact(ParentKey) =/= miss end,
        1000
    ),
    Before = barrel_inference_cache:get_counters(),
    Prompt2 = <<"alpha beta gamma extra">>,
    {ok, _} = barrel_inference_model:complete(Model, Prompt2, #{
...
longest_prefix_resume_without_parent_key(Config) ->
    ...
    {ok, _} = barrel_inference_model:complete(Model, Prompt1, #{response_tokens => 2}),
    timer:sleep(150),
    Before = barrel_inference_cache:get_counters(),
...
warm_read_pins_via_checkout(Config) ->
    Model = ?config(model, Config),
    Prompt = long_prompt(),
    {ok, _} = barrel_inference_model:complete(Model, Prompt, #{response_tokens => 2}),
    timer:sleep(100),
    Before = barrel_inference_cache:get_counters(),
    {ok, _} = barrel_inference_model:complete(Model, Prompt, #{response_tokens => 2}),
    After = barrel_inference_cache:get_counters(),
...
eviction_drops_files_and_meta(Config) ->
    ...
    {ok, _} =
        barrel_inference_model:complete(Model, long_prompt(), #{response_tokens => 4}),
...
concurrent_complete_under_writer_cap(Config) ->
    ...
    Pids = [
        spawn(fun() ->
            ...
            {ok, _} = barrel_inference_model:start_link(ModelN, Cfg),
            try
                ...
                Parent ! {done, I}
            after
                catch barrel_inference_model:stop(ModelN)
            end
        end)
     || I <- lists:seq(1, N)
    ],
...
counters_visible_via_facade(Config) ->
    Model = ?config(model, Config),
    {ok, _} = barrel_inference_model:complete(Model, long_prompt()),
    timer:sleep(100),
    Snap = barrel_inference_cache:get_counters(),
    ?assert(is_map(Snap)),
    ...
    Empty = barrel_inference_cache:get_counters(),
