...
stub_step_decode_is_deterministic_per_seq_and_sampler_test() ->
    {ok, S} = barrel_inference_model_stub:init(#{}),
    {ok, Sampler0} = barrel_inference_model_stub:sampler_new(S, #{}),
    {ok, [{0, {token, T1, 0}}]} =
        barrel_inference_model_stub:step(S, [{0, {decode, Sampler0}}]),
...
stub_step_decode_differs_across_seqs_test() ->
    {ok, S} = barrel_inference_model_stub:init(#{}),
    {ok, Sampler} = barrel_inference_model_stub:sampler_new(S, #{}),
...
stub_sampler_new_returns_unique_refs_test() ->
    {ok, S} = barrel_inference_model_stub:init(#{}),
    {ok, A} = barrel_inference_model_stub:sampler_new(S, #{}),
    {ok, B} = barrel_inference_model_stub:sampler_new(S, #{}),
    ?assertNotEqual(A, B),
    ok = barrel_inference_model_stub:sampler_free(A),
    ok = barrel_inference_model_stub:sampler_free(B).
...
stub_kv_pack_seq_id_matches_legacy_test() ->
    ...
    {ok, S} = barrel_inference_model_stub:init(#{}),
    Tokens = [11, 22, 33, 44],
    Legacy = barrel_inference_model_stub:kv_pack(S, Tokens),
    SeqAware = barrel_inference_model_stub:kv_pack(S, Tokens, 1),
    ?assertEqual(Legacy, SeqAware).

stub_kv_unpack_seq_id_is_noop_test() ->
    {ok, S} = barrel_inference_model_stub:init(#{}),
    ?assertEqual(ok, barrel_inference_model_stub:kv_unpack(S, <<>>, 0)),
    ?assertEqual(ok, barrel_inference_model_stub:kv_unpack(S, <<>>, 1)).
