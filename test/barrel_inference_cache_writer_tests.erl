...
%%
-module(barrel_inference_cache_writer_tests).
...
with_writer(Max, Body) ->
    ...
    {ok, _} = barrel_inference_cache_writer:start_link(Max),
    try
        Body(Dir)
    after
...
save_publishes_via_reservation_protocol_test() ->
    with_writer(4, fun(_Dir) ->
        ...
        Payload = <<"first save">>,
        {ok, Key} = barrel_inference_cache_writer:save(test_disk, disk, Meta, Payload),
...
save_already_present_returns_error_test() ->
    with_writer(4, fun(_Dir) ->
        Meta = base_meta([1, 2, 3]),
        {ok, _Key} = barrel_inference_cache_writer:save(test_disk, disk, Meta, <<"x">>),
        %% Second save for the same Key: meta_srv:reserve_save returns
        %% {error, already_present}; writer propagates it.
        ?assertEqual(
            {error, already_present},
            barrel_inference_cache_writer:save(test_disk, disk, Meta, <<"x">>)
        )
    end).
...
current_and_max_reflect_init_test() ->
    with_writer(7, fun(_Dir) ->
        ?assertEqual(7, barrel_inference_cache_writer:max_concurrent()),
        ?assertEqual(0, barrel_inference_cache_writer:current())
    end).
...
semaphore_returns_max_concurrent_on_timeout_test() ->
    ...
    with_writer(1, fun(_Dir) ->
        ...
        Pid =
            spawn(fun() ->
                ok = barrel_inference_cache_writer:acquire(infinity),
                Parent ! pinned,
                receive
                    release -> barrel_inference_cache_writer:release()
                end
            end),
        receive
            pinned -> ok
        ...
        Pid ! release,
        wait_until(fun() -> barrel_inference_cache_writer:current() =:= 0 end, 1000),
        ?assertEqual(miss, barrel_inference_cache_meta_srv:lookup_exact(Key))
    end).

release_saturates_at_zero_test() ->
    with_writer(2, fun(_Dir) ->
        %% Double-release: the saturating decrement clamps at 0.
        ok = barrel_inference_cache_writer:release(),
        ok = barrel_inference_cache_writer:release(),
        ?assertEqual(0, barrel_inference_cache_writer:current())
    end).
...
down_releases_slot_no_leak_test() ->
    with_writer(1, fun(_Dir) ->
        ...
        ?assertEqual(1, barrel_inference_cache_writer:current()),
        %% Kill the holder without releasing.
        exit(Pid, kill),
        wait_until(fun() -> barrel_inference_cache_writer:current() =:= 0 end, 1000),
        %% A subsequent acquire should succeed.
        ok = barrel_inference_cache_writer:acquire(100),
        barrel_inference_cache_writer:release()
    end).

set_max_concurrent_takes_effect_test() ->
    with_writer(2, fun(_Dir) ->
        ok = barrel_inference_cache_writer:set_max_concurrent(5),
        ?assertEqual(5, barrel_inference_cache_writer:max_concurrent())
    end).
...
bad_meta_cancels_reservation_test() ->
    with_writer(2, fun(_Dir) ->
        ...
        ?assertMatch(
            {error, _},
            barrel_inference_cache_writer:save(test_disk, disk, BadMeta, <<"x">>)
        ),
        %% Verify the reservation was cancelled (not stuck `writing`).
        BadKey = barrel_inference_cache_key:make(#{
            fingerprint => binary:copy(<<16#AA>>, 32),
            quant_type => f16,
            ctx_params_hash => binary:copy(<<16#BB>>, 32),
            tokens => [1, 2, 3]
...
make_tmp_dir() ->
    ...
    Dir = filename:join(
        Base,
        "barrel_inference_cache_writer_tests_" ++
