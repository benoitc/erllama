...
-module(barrel_inference_chat_template_tests).
...
with_model(Body) ->
    with_app(fun(DiskSrv) ->
        ...
        {ok, _} = barrel_inference:load_model(Id, minimal_config(DiskSrv)),
        try
            Body(Id)
        after
...
apply_chat_template_simple_test() ->
    with_model(fun(Id) ->
        ...
        {ok, Tokens} = barrel_inference:apply_chat_template(Id, Request),
...
apply_chat_template_with_system_test() ->
    with_model(fun(Id) ->
        ...
        {ok, A} = barrel_inference:apply_chat_template(Id, Without),
        {ok, B} = barrel_inference:apply_chat_template(Id, With),
...
apply_chat_template_deterministic_test() ->
    with_model(fun(Id) ->
        ...
        {ok, A} = barrel_inference:apply_chat_template(Id, Request),
        {ok, B} = barrel_inference:apply_chat_template(Id, Request),
        ?assertEqual(A, B)
    end).
...
embed_returns_vector_test() ->
    with_model(fun(Id) ->
        {ok, Tokens} = barrel_inference:tokenize(Id, <<"some text">>),
        {ok, Vec} = barrel_inference:embed(Id, Tokens),
        ?assert(is_list(Vec)),
        ?assert(length(Vec) > 0),
...
embed_deterministic_test() ->
    with_model(fun(Id) ->
        ...
        {ok, A} = barrel_inference:embed(Id, Tokens),
        {ok, B} = barrel_inference:embed(Id, Tokens),
        ?assertEqual(A, B)
    end).

embed_different_input_different_vector_test() ->
    with_model(fun(Id) ->
        {ok, T1} = barrel_inference:tokenize(Id, <<"alpha">>),
        {ok, T2} = barrel_inference:tokenize(Id, <<"omega">>),
        {ok, A} = barrel_inference:embed(Id, T1),
        {ok, B} = barrel_inference:embed(Id, T2),
        ?assertNotEqual(A, B)
    end).
...
grammar_undefined_is_no_op_test() ->
    with_model(fun(Id) ->
        ...
        {ok, _Ref} = barrel_inference:infer(Id, Tokens, Params, self()),
        receive
            {barrel_inference_done, _, _} -> ok
...
unsupported_apply_chat_template_returns_error_test() ->
    ...
    with_model(fun(Id) ->
        ?assertMatch(
            {ok, _},
            barrel_inference:apply_chat_template(
                Id,
