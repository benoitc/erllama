...
%%
%% Tests for streaming inference: barrel_inference:infer/4 + barrel_inference:cancel/1.
%% Uses the stub backend so no GGUF model file is required.
-module(barrel_inference_streaming_tests).
...
with_model(ConfigOverrides, Body) ->
    with_app(fun(DiskSrv) ->
        ...
        {ok, _} = barrel_inference:load_model(Id, Cfg),
        try
            Body(Id)
        after
...
make_tmp_dir() ->
    ...
    Dir = filename:join(
        Base,
        "barrel_inference_streaming_tests_" ++
            integer_to_list(erlang:unique_integer([positive]))
    ),
...
%% Drain messages of a given Ref until {barrel_inference_done, Ref, _} or timeout.
%% Returns {Tokens, Stats} or {error, Reason} on barrel_inference_error.
collect_stream(Ref, TimeoutMs) ->
    collect_stream(Ref, TimeoutMs, []).

collect_stream(Ref, TimeoutMs, Acc) ->
    receive
        {barrel_inference_token, Ref, Bin} ->
...
infer_returns_ref_and_streams_tokens_test() ->
    with_model(fun(Id) ->
        ...
        {ok, Ref} = barrel_inference:infer(Id, Tokens, #{response_tokens => 4}, self()),
...
infer_emits_binary_fragments_test() ->
    with_model(fun(Id) ->
        {ok, Tokens} = barrel_inference:tokenize(Id, <<"x y z">>),
        {ok, Ref} = barrel_inference:infer(Id, Tokens, #{response_tokens => 3}, self()),
...
infer_finish_reason_length_when_target_reached_test() ->
    with_model(fun(Id) ->
        {ok, Tokens} = barrel_inference:tokenize(Id, <<"a b">>),
...
stats_carry_token_counts_test() ->
    with_model(fun(Id) ->
        {ok, PromptTokens} = barrel_inference:tokenize(Id, <<"hello big world">>),
        {ok, Ref} = barrel_inference:infer(Id, PromptTokens, #{response_tokens => 4}, self()),
        {Out, Stats} = collect_stream(Ref, 5000),
...
stats_carry_finish_key_and_committed_tokens_test() ->
    with_model(fun(Id) ->
        {ok, PromptTokens} = barrel_inference:tokenize(Id, <<"hello big world">>),
        {ok, Ref} = barrel_inference:infer(Id, PromptTokens, #{response_tokens => 6}, self()),
...
stop_sequence_value_reported_on_match_test() ->
    with_model(fun(Id) ->
        ...
        {ok, Ref} = barrel_inference:infer(Id, Tokens, Params, self()),
        {_Out, Stats} = collect_stream(Ref, 5000),
...
collect_all(Ref, TimeoutMs, Acc) ->
    receive
        ...
        {barrel_inference_thinking_end, Ref, Sig} ->
            collect_all(Ref, TimeoutMs, [{thinking_end, Sig} | Acc]);
        {barrel_inference_tool_call_end, Ref, Full} ->
...
thinking_capable_stub_with_thinking_disabled_fails_test() ->
    ...
    %% clean barrel_inference_error tagged thinking_not_enabled.
    with_model(#{thinking_capable => true}, fun(Id) ->
        {ok, Tokens} = barrel_inference:tokenize(Id, <<"hello">>),
        {ok, Ref} = barrel_inference:infer(Id, Tokens, #{response_tokens => 3}, self()),
        Events = collect_all(Ref, 5000),
...
grammar_request_disables_greedy_on_syntax_test() ->
    with_model(#{tool_call_capable => true}, fun(Id) ->
        barrel_inference_model_stub:reset_sampler_new_cfgs(),
        try
            {ok, Tokens} = barrel_inference:tokenize(Id, <<"hello">>),
            GBNF = <<"root ::= \"x\"">>,
            {ok, Ref} = barrel_inference:infer(
            ...
            Cfgs = barrel_inference_model_stub:sampler_new_cfgs(),
...
second_infer_while_busy_is_queued_test() ->
    ...
    with_model(fun(Id) ->
        {ok, Tokens} = barrel_inference:tokenize(Id, <<"hello">>),
        {ok, Ref1} = barrel_inference:infer(Id, Tokens, #{response_tokens => 10000}, self()),
        receive
...
complete_while_streaming_is_queued_test() ->
    ...
    with_model(fun(Id) ->
        {ok, Tokens} = barrel_inference:tokenize(Id, <<"hi">>),
        {ok, Ref} = barrel_inference:infer(Id, Tokens, #{response_tokens => 10000}, self()),
        receive
        ...
        Parent = self(),
        spawn(fun() ->
            Parent ! {complete_result, barrel_inference:complete(Id, <<"another">>)}
        end),
...
queued_requests_drain_in_fifo_order_test() ->
    ...
    %% the second and third are queued. The barrel_inference_done messages
    %% must arrive in submission order (FIFO queue semantics).
    with_model(fun(Id) ->
        {ok, Tokens} = barrel_inference:tokenize(Id, <<"hi">>),
        {ok, R1} = barrel_inference:infer(Id, Tokens, #{response_tokens => 2}, self()),
        {ok, R2} = barrel_inference:infer(Id, Tokens, #{response_tokens => 2}, self()),
        {ok, R3} = barrel_inference:infer(Id, Tokens, #{response_tokens => 2}, self()),
...
drain_done_in_order(N, Timeout) ->
    receive
        {barrel_inference_done, Ref, _} -> [Ref | drain_done_in_order(N - 1, Timeout)];
        %% Tokens emitted while waiting for done are fine; ignore them.
        {barrel_inference_token, _, _} -> drain_done_in_order(N, Timeout)
...
cancel_unknown_ref_is_noop_test() ->
    with_app(fun(_DiskSrv) ->
        ?assertEqual(ok, barrel_inference:cancel(make_ref()))
    end).

cancel_idempotent_test() ->
    with_app(fun(_DiskSrv) ->
        Ref = make_ref(),
        ?assertEqual(ok, barrel_inference:cancel(Ref)),
        ?assertEqual(ok, barrel_inference:cancel(Ref))
    end).
...
cancelled_stream_releases_model_for_next_request_test() ->
    with_model(fun(Id) ->
        ...
        %% accept a new infer.
        {ok, Ref2} = barrel_inference:infer(Id, Tokens, #{response_tokens => 2}, self()),
...
inflight_registers_and_unregisters_test() ->
    with_model(fun(Id) ->
        {ok, Tokens} = barrel_inference:tokenize(Id, <<"x">>),
        Pre = length(barrel_inference_inflight:all()),
        {ok, Ref} = barrel_inference:infer(Id, Tokens, #{response_tokens => 10000}, self()),
        receive
            {barrel_inference_token, Ref, _} -> ok
        after 5000 -> ?assert(false)
        end,
        ?assertMatch({ok, _Pid}, barrel_inference_inflight:lookup(Ref)),
        ?assert(length(barrel_inference_inflight:all()) >= Pre + 1),
        ok = barrel_inference:cancel(Ref),
        _ = collect_stream(Ref, 5000),
        ?assertEqual({error, not_found}, barrel_inference_inflight:lookup(Ref))
    end).
