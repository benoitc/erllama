...
pin_protects_row_from_eviction_test() ->
    with_srv(fun() ->
        Ns = ns(1),
        ok = erllama_cache_meta_srv:insert_available(key(1), ram, 100, <<"H">>, {ram}),
        ok = erllama_cache_meta_srv:insert_available(key(2), ram, 100, <<"H">>, {ram}),
        ok = erllama_cache_meta_srv:pin_row(Ns, key(1)),
        {ok, R1} = erllama_cache_meta_srv:dump(key(1)),
...
frecency_evicts_coldest_first_test() ->
    with_srv(fun() ->
        ...
        {ok, Ref, _, _, _, _} = erllama_cache_meta_srv:checkout(key(2), self()),
        ok = erllama_cache_meta_srv:checkin(Ref),
        {evicted, 1, 100} = erllama_cache_meta_srv:evict_bytes(100),
        ?assertEqual(miss, erllama_cache_meta_srv:lookup_exact(key(1))),
...
live_key_protected_from_eviction_test() ->
    with_srv(fun() ->
        ok = erllama_cache_meta_srv:insert_available(key(1), ram, 100, <<"H">>, {ram}),
        ok = erllama_cache_meta_srv:insert_available(key(2), ram, 100, <<"H">>, {ram}),
        ok = erllama_cache_meta_srv:set_live_key(key(1)),
        timer:sleep(20),
        {evicted, 1, 100} = erllama_cache_meta_srv:evict_bytes(100),
        ?assertMatch({ok, _}, erllama_cache_meta_srv:lookup_exact(key(1))),
        ?assertEqual(miss, erllama_cache_meta_srv:lookup_exact(key(2))),
...
pin_new_key_unpins_prior_for_namespace_test() ->
    with_srv(fun() ->
        ...
        ok = erllama_cache_meta_srv:insert_available(key(2), ram, 100, <<"H">>, {ram}),
        ok = erllama_cache_meta_srv:pin_row(Ns, key(1)),
        ok = erllama_cache_meta_srv:pin_row(Ns, key(2)),
        {ok, R1} = erllama_cache_meta_srv:dump(key(1)),
        {ok, R2} = erllama_cache_meta_srv:dump(key(2)),
...
insert_text_row(Meta, TextPrefix) ->
    ...
    K = erllama_cache_key:make_text(Fp, QT, Ctx, TextPrefix),
    ok = erllama_cache_meta_srv:insert_available(
...
lookup_longest_text_prefix_miss_test() ->
    with_srv(fun() ->
        ...
        ?assertEqual(
            miss,
            erllama_cache_meta_srv:lookup_longest_text_prefix(
...
announce_saved_promotes_to_available_test() ->
    with_srv(fun() ->
        ...
        ok = erllama_cache_meta_srv:announce_saved(
