...
grammar_cache_reuses_compiled_template(Config) ->
    ...
    try
        {ok, Ctx} = erllama_nif:new_context(Model, #{n_ctx => 512}),
        G = <<"root ::= \"yes\" | \"no\"">>,
        ok = erllama_nif:configure_sampler(Ctx, #{grammar => G}),
        ?assertMatch(#{hits := 0, misses := 1}, erllama_nif:grammar_cache_stats(Ctx)),
        ok = erllama_nif:configure_sampler(Ctx, #{grammar => G}),
        ?assertMatch(#{hits := 1, misses := 1}, erllama_nif:grammar_cache_stats(Ctx)),
