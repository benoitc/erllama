...
%%
-module(prop_barrel_inference_cache_policy).
...
prop_trimmed_length_is_aligned() ->
    ?FORALL(
        ...
        case barrel_inference_cache_policy:trim_boundary(Tokens, Trim, Align) of
            {ok, Prefix} ->
...
prop_cold_split_concat_equals_input() ->
    ?FORALL(
        ...
        case barrel_inference_cache_policy:cold_save_split(Tokens, Cfg) of
...
prop_validate_config_accepts_valid_cfg() ->
    ?FORALL(
        Cfg,
        valid_cfg(),
        ok =:= barrel_inference_cache_policy:validate_config(Cfg)
    ).
