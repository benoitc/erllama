...
-module(barrel_inference_lora_tests).
-include_lib("eunit/include/eunit.hrl").

with_app(Body) ->
    {ok, Started} = application:ensure_all_started(barrel_inference),
    Dir = make_tmp_dir(),
    {ok, _} = barrel_inference_cache_disk_srv:start_link(lora_disk, Dir),
    try
        Body()
    after
...
make_tmp_dir() ->
    Base = filename:basedir(user_cache, "barrel_inference-lora-tests"),
...
write_fixture(Bytes) ->
    Dir = filename:basedir(user_cache, "barrel_inference-lora-fixtures"),
    ok = filelib:ensure_path(Dir),
    Path = filename:join(
...
load_unload_round_trip_test() ->
    with_app(fun() ->
        Id = <<"lora_a">>,
        {ok, _} = barrel_inference:load_model(Id, minimal_config()),
        try
            Path = write_fixture(<<"adapter-bytes-1">>),
            {ok, H} = barrel_inference:load_adapter(Id, Path),
            ?assert(is_reference(H)),
            ?assertMatch([_], barrel_inference:list_adapters(Id)),
            ok = barrel_inference:unload_adapter(Id, H),
            ?assertEqual([], barrel_inference:list_adapters(Id))
        after
...
apply_adapters_records_pairs_test() ->
    with_app(fun() ->
        ...
        try
            ...
            {ok, H1} = barrel_inference:load_adapter(Id, P1),
            {ok, H2} = barrel_inference:load_adapter(Id, P2),
            Applied = barrel_inference_model_stub:applied_adapters(
                barrel_inference_model:get_backend_state(Id)
            ),
...
scale_change_replays_pairs_test() ->
    with_app(fun() ->
        ...
        try
            ...
            {ok, H} = barrel_inference:load_adapter(Id, Path),
            [{H, 1.0}] =
                barrel_inference_model_stub:applied_adapters(
                    barrel_inference_model:get_backend_state(Id)
                ),
            ok = barrel_inference:set_adapter_scale(Id, H, 0.5),
            [{H, 0.5}] =
                barrel_inference_model_stub:applied_adapters(
                    barrel_inference_model:get_backend_state(Id)
                )
...
unload_unknown_handle_is_idempotent_test() ->
    with_app(fun() ->
        ...
        {ok, _} = barrel_inference:load_model(Id, minimal_config()),
        try
            ?assertEqual(ok, barrel_inference:unload_adapter(Id, make_ref()))
        after
            barrel_inference:unload(Id)
        end
    end).
...
effective_fingerprint_segregates_cache_keys_test() ->
    ...
    Fp0 = barrel_inference_cache_key:effective_fingerprint(Base, []),
    FpA = barrel_inference_cache_key:effective_fingerprint(Base, [{Sha1, 1.0}]),
    FpB = barrel_inference_cache_key:effective_fingerprint(Base, [{Sha2, 1.0}]),
    FpAScaled = barrel_inference_cache_key:effective_fingerprint(Base, [{Sha1, 0.5}]),
...
effective_fingerprint_sort_invariance_test() ->
    ...
    Fp1 = barrel_inference_cache_key:effective_fingerprint(Base, [A, B]),
    Fp2 = barrel_inference_cache_key:effective_fingerprint(Base, [B, A]),
    ?assertEqual(Fp1, Fp2).
