...
%%
-module(prop_barrel_inference_cache_kvc).
...
prop_tokens_round_trip() ->
    ?FORALL(
        ...
        begin
            {ok, Prefix} = barrel_inference_cache_kvc:build(Meta, Payload),
            Bin = <<Prefix/binary, Payload/binary>>,
            {ok, Info} = barrel_inference_cache_kvc:parse_meta(Bin),
    ...
    ?FORALL(
        ...
        begin
            ...
            CorrectKey =:= WrongKey orelse
                element(1, barrel_inference_cache_kvc:parse(Bin, WrongKey)) =:= error
        end
    ).
