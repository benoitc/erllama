...
model_unload_escalates_when_cache_empty_test() ->
    ...
    try
        with_scheduler(model_unload_config(), fun() ->
            ...
            ?assertEqual({unloaded_model, <<"m1">>}, erllama_scheduler:force_check()),
            S = erllama_scheduler:status(),
...
model_unload_none_when_no_idle_test() ->
    ...
    try
        with_scheduler(model_unload_config(), fun() ->
            ?assertEqual({skipped, nothing_to_evict}, erllama_scheduler:force_check()),
            ?assertEqual(0, maps:get(models_unloaded_total, erllama_scheduler:status()))
        end)
    after
        evict_ret_clear()
    end.
...
cache_relief_skips_model_unload_test() ->
    ...
    try
        with_scheduler(model_unload_config(), fun() ->
            ...
            ?assertMatch({evicted, _, _}, erllama_scheduler:force_check()),
            ?assertEqual(0, maps:get(models_unloaded_total, erllama_scheduler:status()))
        end)
    after
        evict_ret_clear()
...
invalid_unload_flag_at_init_test() ->
    ?assertMatch(
        ...
        erllama_scheduler:validate_config(#{unload_models_under_pressure => yes})
    ).

invalid_model_evictor_at_init_test() ->
    ?assertMatch(
        {error, {invalid_config, {model_evictor, _}}},
        erllama_scheduler:validate_config(#{model_evictor => "not_an_atom"})
    ).
