...
-module(barrel_inference_bench_collect).
...
main(_) ->
    io:format(standard_error, "usage: barrel_inference_bench_collect <model-path> <out-json>~n", []),
    halt(2).
...
run(ModelPath, OutPath) ->
    case filelib:is_regular(ModelPath) of
        ...
        true ->
            ...
            {ok, _} = barrel_inference:load_model(ModelId, Config),
            LoadMs = erlang:monotonic_time(millisecond) - LoadT0,
            Workloads =
                try
                    run_workloads(ModelId)
                after
                    catch barrel_inference:unload(ModelId),
            ...
            Doc = #{
                ...
                host => host_meta(),
                gpu => gpu_meta(),
                barrel_inference => barrel_inference_meta(),
...
warmup(Model, TargetTokens) ->
    ...
    _ = barrel_inference_model:complete(Model, Prompt, #{response_tokens => 4}),
    ok.
...
reset_cache() ->
    catch barrel_inference_cache_meta_srv:gc(),
    catch barrel_inference_cache_counters:reset(),
    ok.

measure_complete(Model, Name, Prompt, RespTokens) ->
    {ok, Result} = barrel_inference_model:complete(Model, Prompt, #{response_tokens => RespTokens}),
...
measure_continue_3turn(Model, ShortPrompt, RespTokens) ->
    ...
    {ok, PromptTokens} = barrel_inference:tokenize(Model, ShortPrompt),
...
drain_done(Ref, TimeoutMs) ->
    receive
        ...
        {barrel_inference_error, Ref, R} -> erlang:error({stream_error, R})
    after TimeoutMs ->
...
host_meta() ->
    #{
        ...
        os_kernel => binstr(os:getenv("BARREL_INFERENCE_BENCH_OS_KERNEL", "")),
        os_release => binstr(os:getenv("BARREL_INFERENCE_BENCH_OS_RELEASE", "")),
        arch => binstr(os:getenv("BARREL_INFERENCE_BENCH_ARCH", "")),
        cpu_brand => binstr(os:getenv("BARREL_INFERENCE_BENCH_CPU_BRAND", "")),
        physical_cores => env_int("BARREL_INFERENCE_BENCH_PHYSICAL_CORES", 0),
        ram_mb => env_int("BARREL_INFERENCE_BENCH_RAM_MB", 0)
    }.

gpu_meta() ->
    %% Populated by the shell wrapper (`bench/collect.sh`), which knows
    %% how to probe nvidia-smi / rocm-smi / system_profiler.
    Kind = binstr(os:getenv("BARREL_INFERENCE_BENCH_GPU_KIND", "cpu")),
    #{
        kind => Kind,
        name => binstr(os:getenv("BARREL_INFERENCE_BENCH_GPU_NAME", "")),
        memory_mb => env_int("BARREL_INFERENCE_BENCH_GPU_MEMORY_MB", 0),
        driver => binstr(os:getenv("BARREL_INFERENCE_BENCH_GPU_DRIVER", ""))
    }.

barrel_inference_meta() ->
    Vsn =
        case application:get_key(barrel_inference, vsn) of
            {ok, V} -> binstr(V);
            _ -> <<"unknown">>
        end,
    #{
        ...
        git_commit => binstr(os:getenv("BARREL_INFERENCE_BENCH_GIT_COMMIT", "")),
        git_dirty => binstr(os:getenv("BARREL_INFERENCE_BENCH_GIT_DIRTY", "unknown"))
    }.
...
model_meta(Path) ->
    ...
    Sha = binstr(os:getenv("BARREL_INFERENCE_BENCH_MODEL_SHA256", "")),
    #{
        path => binstr(Path),
...
ensure_app_started() ->
    application:set_env(barrel_inference, scheduler, #{enabled => false}),
...
model_config(Path, DiskSrv) ->
    ...
    Fp =
        case os:getenv("BARREL_INFERENCE_BENCH_MODEL_SHA256") of
...
make_tmp_dir(Tag) ->
    ...
    Dir = filename:join(Base, "barrel_inference_" ++ Tag),
...
is_nif_loaded() ->
    try
        barrel_inference_nif:module_info(exports),
        true
    catch
        _:_ -> false
    end.
