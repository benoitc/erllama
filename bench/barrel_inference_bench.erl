...
cold_vs_warm(ModelPath, PromptTokenTargets) ->
    ...
    {ok, _} = barrel_inference_model:start_link(Model, model_config(ModelPath, DiskSrv)),
    try
    ...
    after
        catch barrel_inference_model:stop(Model),
...
run_cold_vs_warm_at(Model, TokenTarget) ->
    ...
    barrel_inference_cache:reset_counters(),
    {ColdUs, _} = timer:tc(
        barrel_inference_model, complete, [Model, Prompt, #{response_tokens => 1}]
    ),
    wait_for_save(2000),
    barrel_inference_cache:reset_counters(),
    {WarmUs, _} = timer:tc(
...
multi_agent(ModelPath, NAgents, SharedTokens) ->
    ...
    {ok, _} = barrel_inference_model:start_link(PrewarmModel, model_config(ModelPath, DiskSrv)),
    try
        %% Pre-warm: cold completion that publishes the cache row.
        {_, _, _} = call_complete(PrewarmModel, SharedPrompt, 1),
        catch barrel_inference_model:stop(PrewarmModel),
        timer:sleep(300),
        barrel_inference_cache:reset_counters(),
        Tasks = [
...
run_one_agent(ModelPath, DiskSrv, I, Task, Parent, Tag) ->
    ...
    {ok, _} = barrel_inference_model:start_link(ModelName, model_config(ModelPath, DiskSrv)),
    try
        {Lat, _} = timer:tc(barrel_inference_model, complete, [ModelName, Task, #{response_tokens => 1}]),
        Parent ! {agent_done, self(), Lat}
    after
        catch barrel_inference_model:stop(ModelName)
    end.
...
ensure_app_started() ->
    ...
    application:ensure_all_started(barrel_inference).

call_complete(Model, Prompt, NTokens) ->
    barrel_inference_model:complete(Model, Prompt, #{response_tokens => NTokens}).
