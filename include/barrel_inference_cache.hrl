...
%%
%% barrel_inference_cache: shared records, type aliases, and ETS row position constants.
%%
%% Position constants are referenced from barrel_inference_cache_meta_srv via
%% ets:update_counter / ets:update_element on a flat tuple row.

-ifndef(BARREL_INFERENCE_CACHE_HRL).
-define(BARREL_INFERENCE_CACHE_HRL, true).
...
%% Bumped when barrel_inference_cache_writer:save returns {error, max_concurrent}
