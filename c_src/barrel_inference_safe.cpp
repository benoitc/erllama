...
// barrel_inference_safe: C++-side shims around llama.cpp calls that can throw.
//
...
#ifndef BARREL_INFERENCE_THREAD_LOCAL
#  if defined(__cplusplus) && __cplusplus >= 201103L
#    define BARREL_INFERENCE_THREAD_LOCAL thread_local
#  else
#    define BARREL_INFERENCE_THREAD_LOCAL __thread
#  endif
#endif

// Sentinel returned by barrel_inference_safe_decode on a thrown exception.
// Mirrors the macro in barrel_inference_nif.c; both sides must agree.
#define BARREL_INFERENCE_DECODE_EXC_SENTINEL INT_MIN

// Model-load classification. Mirrored in barrel_inference_nif.c; both
// sides must agree on the integer values.
typedef enum {
    BARREL_INFERENCE_LOAD_OK        = 0,
    BARREL_INFERENCE_LOAD_FAILED    = 1,  // generic NULL return
    BARREL_INFERENCE_LOAD_MALFORMED = 2,  // captured ASSERT-style log line
    BARREL_INFERENCE_LOAD_EXCEPTION = 3,  // C++ exception caught
...
static BARREL_INFERENCE_THREAD_LOCAL char t_last_log_line[512] = {0};

static void barrel_inference_log_capture(enum ggml_log_level level,
...
barrel_inference_safe_sampler_chain_init(struct llama_sampler_chain_params p) noexcept {
    try {
...
struct llama_sampler *barrel_inference_safe_sampler_init_greedy(void) noexcept {
    try {
...
struct llama_sampler *barrel_inference_safe_sampler_init_dist(uint32_t seed) noexcept {
    try {
...
struct llama_sampler *barrel_inference_safe_sampler_init_top_k(int32_t k) noexcept {
    try {
...
struct llama_sampler *barrel_inference_safe_sampler_init_top_p(float p,
...
struct llama_sampler *barrel_inference_safe_sampler_init_min_p(float p,
                                                     size_t min_keep) noexcept {
    try {
...
struct llama_sampler *barrel_inference_safe_sampler_init_temp(float t) noexcept {
    try {
...
struct llama_sampler *
barrel_inference_safe_sampler_init_penalties(int32_t last_n, float repeat, float freq,
...
int barrel_inference_safe_sampler_chain_add(struct llama_sampler *chain,
...
int barrel_inference_safe_sampler_free(struct llama_sampler *s) noexcept {
    if (!s) return 0;
    try {
...
llama_token barrel_inference_safe_sampler_sample(struct llama_sampler *s,
...
int barrel_inference_safe_sampler_accept(struct llama_sampler *s,
...
int32_t barrel_inference_safe_token_to_piece(const struct llama_vocab *vocab,
...
int barrel_inference_safe_backend_init(void) noexcept {
    try {
        llama_backend_init();
        return 0;
    } catch (...) {
...
int barrel_inference_safe_backend_init_once(void) noexcept {
    ...
    pthread_once(&once, []() noexcept {
        try {
            ...
            // classification. See barrel_inference_log_capture comment.
            llama_log_set(barrel_inference_log_capture, nullptr);
            rc = 0;
        } catch (...) {
            rc = -1;
        }
...
int barrel_inference_safe_backend_free(void) noexcept {
    try {
        llama_backend_free();
        return 0;
    } catch (...) {
...
void barrel_inference_safe_log_unset(void) noexcept {
    try {
...
struct llama_model *
barrel_inference_safe_model_load_from_file(const char *path,
...
struct llama_model *
barrel_inference_safe_model_load_from_file_v2(const char *path,
                                     struct llama_model_params params,
                                     barrel_inference_load_status_t *out_status) noexcept {
    if (out_status) *out_status = BARREL_INFERENCE_LOAD_FAILED;
    t_last_log_line[0] = '\0';
    ...
    try {
        ...
        if (out_status) *out_status = BARREL_INFERENCE_LOAD_EXCEPTION;
        return nullptr;
    }
    if (m) {
        if (out_status) *out_status = BARREL_INFERENCE_LOAD_OK;
        return m;
    }

    bool malformed = strstr(t_last_log_line, "GGML_ASSERT") != nullptr;
    if (out_status) {
        *out_status = malformed ? BARREL_INFERENCE_LOAD_MALFORMED : BARREL_INFERENCE_LOAD_FAILED;
    }
    return nullptr;
}

int barrel_inference_safe_model_free(struct llama_model *m) noexcept {
    if (!m) return 0;
    try {
        llama_model_free(m);
        return 0;
...
barrel_inference_safe_init_from_model(struct llama_model *m,
...
int barrel_inference_safe_free(struct llama_context *c) noexcept {
    if (!c) return 0;
    try {
        llama_free(c);
        return 0;
...
void barrel_inference_safe_set_abort_callback(struct llama_context *c,
                                     bool (*cb)(void *),
                                     void *data) noexcept {
    try {
...
barrel_inference_safe_get_model(const struct llama_context *c) noexcept {
    try {
...
barrel_inference_safe_model_get_vocab(const struct llama_model *m) noexcept {
    try {
...
int32_t barrel_inference_safe_vocab_n_tokens(const struct llama_vocab *v) noexcept {
    try {
...
// "unknown").
uint64_t barrel_inference_safe_model_size(const struct llama_model *m) noexcept {
    try {
...
int32_t barrel_inference_safe_model_n_layer(const struct llama_model *m) noexcept {
    try {
...
uint32_t barrel_inference_safe_n_ctx(const struct llama_context *c) noexcept {
    try {
...
uint32_t barrel_inference_safe_n_batch(const struct llama_context *c) noexcept {
    try {
...
size_t barrel_inference_safe_backend_dev_count(void) noexcept {
    try {
...
int barrel_inference_safe_backend_dev_info(size_t idx, size_t *free_b,
...
int barrel_inference_safe_vocab_is_eog(const struct llama_vocab *v,
                              llama_token tok) noexcept {
    try {
...
int32_t barrel_inference_safe_tokenize(const struct llama_vocab *vocab,
...
// BARREL_INFERENCE_DECODE_EXC_SENTINEL (INT_MIN) on a thrown exception.
int barrel_inference_safe_decode(struct llama_context *c,
                        struct llama_batch batch) noexcept {
    try {
        return llama_decode(c, batch);
    } catch (...) {
        return BARREL_INFERENCE_DECODE_EXC_SENTINEL;
    }
}
...
struct llama_batch barrel_inference_safe_batch_init(int32_t n_tokens, int32_t embd,
...
struct llama_batch barrel_inference_safe_batch_get_one(llama_token *tokens,
...
void barrel_inference_safe_batch_free(struct llama_batch batch) noexcept {
    try {
...
// as failure).
size_t barrel_inference_safe_state_seq_get_size(struct llama_context *c,
                                       int seq_id) noexcept {
    try {
...
size_t barrel_inference_safe_state_seq_get_data(struct llama_context *c,
...
size_t barrel_inference_safe_state_seq_set_data(struct llama_context *c,
...
int barrel_inference_safe_memory_seq_rm(struct llama_context *c, int seq_id,
...
long barrel_inference_safe_memory_seq_pos_max(struct llama_context *c,
                                     int seq_id) noexcept {
    try {
        llama_memory_t mem = llama_get_memory(c);
        if (!mem) return -2;
...
// the model vocab at each position. Used by barrel_inference:verify/4.
//
...
int barrel_inference_safe_forward_with_argmax(struct llama_context *c,
...
const char *barrel_inference_safe_model_chat_template(const struct llama_model *m,
...
// exception.
int32_t barrel_inference_safe_chat_apply_template(const char *tmpl,
...
struct llama_sampler *
barrel_inference_safe_sampler_init_grammar(const struct llama_vocab *vocab,
...
barrel_inference_safe_adapter_lora_init(struct llama_model *model,
...
void barrel_inference_safe_adapter_lora_free(struct llama_adapter_lora *a) noexcept {
    if (!a) return;
    try {
...
int barrel_inference_safe_set_adapters_lora(struct llama_context *ctx,
...
float *barrel_inference_safe_get_embeddings_seq(struct llama_context *c,
                                       int seq_id) noexcept {
    try {
...
float *barrel_inference_safe_get_embeddings(struct llama_context *c) noexcept {
    try {
...
int32_t barrel_inference_safe_n_embd(const struct llama_model *m) noexcept {
    try {
...
int barrel_inference_safe_set_embeddings(struct llama_context *c, bool value) noexcept {
    try {
