...
/*
 * barrel_inference_nif: single NIF for barrel_inference (cache + llama.cpp surface).
 *
 * v0.2 surface:
 ...
 * (currently 0/1/-1/2). Defined here and in barrel_inference_safe.cpp; both
 * sides must agree. */
#define BARREL_INFERENCE_DECODE_EXC_SENTINEL INT_MIN
 ...
 * array on `barrel_inference_context_t`. Sized well above realistic loads
...
#define BARREL_INFERENCE_N_SEQ_MAX_CAP 256

/* Mirror of barrel_inference_load_status_t in barrel_inference_safe.cpp; both sides
...
typedef enum {
    BARREL_INFERENCE_LOAD_OK        = 0,
    BARREL_INFERENCE_LOAD_FAILED    = 1,
    BARREL_INFERENCE_LOAD_MALFORMED = 2,
    BARREL_INFERENCE_LOAD_EXCEPTION = 3,
} barrel_inference_load_status_t;

#include "crc32c.h"
#include "llama.h"

/* Exception-safe wrappers for llama.cpp calls that can throw across
 * the C ABI. Implemented in c_src/barrel_inference_safe.cpp. Each returns a
 ...
 * a C frame. */
extern struct llama_sampler *barrel_inference_safe_sampler_chain_init(
    struct llama_sampler_chain_params p);
extern struct llama_sampler *barrel_inference_safe_sampler_init_greedy(void);
extern struct llama_sampler *barrel_inference_safe_sampler_init_dist(uint32_t seed);
extern struct llama_sampler *barrel_inference_safe_sampler_init_top_k(int32_t k);
extern struct llama_sampler *barrel_inference_safe_sampler_init_top_p(float p,
                                                             size_t min_keep);
extern struct llama_sampler *barrel_inference_safe_sampler_init_min_p(float p,
                                                             size_t min_keep);
extern struct llama_sampler *barrel_inference_safe_sampler_init_temp(float t);
extern struct llama_sampler *barrel_inference_safe_sampler_init_penalties(
...
extern llama_token barrel_inference_safe_sampler_sample(struct llama_sampler *s,
                                                ...
                                                int32_t idx);
extern int barrel_inference_safe_sampler_accept(struct llama_sampler *s,
                                       llama_token tok);
extern int32_t barrel_inference_safe_token_to_piece(const struct llama_vocab *vocab,
                                           llama_token tok, char *buf,
                                           int32_t buf_size,
                                           int32_t lstrip,
                                           bool special);
extern int barrel_inference_safe_backend_init(void);
extern int barrel_inference_safe_backend_init_once(void);
extern int barrel_inference_safe_backend_free(void);
extern void barrel_inference_safe_log_unset(void);
extern struct llama_model *barrel_inference_safe_model_load_from_file(
    const char *path, struct llama_model_params params);
extern struct llama_model *barrel_inference_safe_model_load_from_file_v2(
    const char *path, struct llama_model_params params,
    barrel_inference_load_status_t *out_status);
extern int barrel_inference_safe_model_free(struct llama_model *m);
extern struct llama_context *barrel_inference_safe_init_from_model(
    struct llama_model *m, struct llama_context_params params);
extern int barrel_inference_safe_free(struct llama_context *c);
extern void barrel_inference_safe_set_abort_callback(
    struct llama_context *c, bool (*cb)(void *), void *data);
extern const struct llama_model *barrel_inference_safe_get_model(
    const struct llama_context *c);
extern const struct llama_vocab *barrel_inference_safe_model_get_vocab(
    const struct llama_model *m);
extern int32_t barrel_inference_safe_vocab_n_tokens(const struct llama_vocab *v);
extern uint64_t barrel_inference_safe_model_size(const struct llama_model *m);
extern int32_t barrel_inference_safe_model_n_layer(const struct llama_model *m);
extern uint32_t barrel_inference_safe_n_ctx(const struct llama_context *c);
extern uint32_t barrel_inference_safe_n_batch(const struct llama_context *c);
extern size_t barrel_inference_safe_backend_dev_count(void);
extern int barrel_inference_safe_backend_dev_info(size_t idx, size_t *free_b,
                                         size_t *total_b, int *dev_type);
extern int barrel_inference_safe_vocab_is_eog(const struct llama_vocab *v,
                                     llama_token tok);
extern int32_t barrel_inference_safe_tokenize(const struct llama_vocab *vocab,
                                     const char *text, int32_t text_len,
                                     llama_token *tokens, int32_t n_max,
                                     bool add_special, bool parse_special);
extern int barrel_inference_safe_decode(struct llama_context *c,
                               struct llama_batch batch);
extern struct llama_batch barrel_inference_safe_batch_init(int32_t n_tokens,
                                                  int32_t embd,
                                                  int32_t n_seq_max);
extern struct llama_batch barrel_inference_safe_batch_get_one(llama_token *tokens,
                                                     int32_t n_tokens);
extern void barrel_inference_safe_batch_free(struct llama_batch batch);
extern size_t barrel_inference_safe_state_seq_get_size(struct llama_context *c,
                                              int seq_id);
extern size_t barrel_inference_safe_state_seq_get_data(struct llama_context *c,
                                              uint8_t *dst, size_t size,
                                              int seq_id);
extern size_t barrel_inference_safe_state_seq_set_data(struct llama_context *c,
                                              const uint8_t *src,
                                              size_t size, int seq_id);
extern int barrel_inference_safe_memory_seq_rm(struct llama_context *c, int seq_id,
                                      int p0, int p1);
extern long barrel_inference_safe_memory_seq_pos_max(struct llama_context *c,
                                            int seq_id);
extern int barrel_inference_safe_forward_with_argmax(struct llama_context *c,
                                            const llama_token *tokens,
                                            int32_t n_tokens,
                                            int32_t n_vocab,
                                            long start_pos,
                                            int32_t *out_argmax);
extern const char *barrel_inference_safe_model_chat_template(const struct llama_model *m,
                                                    const char *name);
extern int32_t barrel_inference_safe_chat_apply_template(
    const char *tmpl, const struct llama_chat_message *msgs, size_t n_msgs,
    bool add_assistant, char *buf, int32_t buf_size);
extern struct llama_sampler *barrel_inference_safe_sampler_init_grammar(
    const struct llama_vocab *vocab, const char *grammar_str,
    const char *grammar_root);
extern struct llama_adapter_lora *barrel_inference_safe_adapter_lora_init(
    struct llama_model *model, const char *path);
extern void barrel_inference_safe_adapter_lora_free(struct llama_adapter_lora *a);
extern int barrel_inference_safe_set_adapters_lora(struct llama_context *ctx,
                                          struct llama_adapter_lora **adapters,
                                          size_t n_adapters, float *scales);
extern float *barrel_inference_safe_get_embeddings_seq(struct llama_context *c,
                                              int seq_id);
extern float *barrel_inference_safe_get_embeddings(struct llama_context *c);
extern int32_t barrel_inference_safe_n_embd(const struct llama_model *m);
extern int barrel_inference_safe_set_embeddings(struct llama_context *c, bool value);

#ifndef BARREL_INFERENCE_MAX_TOKENS
 ...
 * build time via -DBARREL_INFERENCE_MAX_TOKENS=N. */
#define BARREL_INFERENCE_MAX_TOKENS (16 * 1024 * 1024)
#endif

#ifndef BARREL_INFERENCE_MAX_TOKEN_TEXT
 ...
 * downstream barrel_inference_server's max_request_body_bytes so any input
 * the server passes through is bounded by this cap. Override at
 * build time via -DBARREL_INFERENCE_MAX_TOKEN_TEXT=N for batch workflows. */
#define BARREL_INFERENCE_MAX_TOKEN_TEXT (64 * 1024 * 1024)
#endif
...
typedef struct {
    ...
    barrel_inference_model_t *model_res;    /* keep_resource'd by new_context */
...
typedef struct {
    ...
    barrel_inference_model_t *model_res;         /* keep_resource'd at init */
} barrel_inference_adapter_t;
...
static void context_drops_model(barrel_inference_model_t *m) {
    ...
    if (m->release_pending && m->active_contexts == 0
        ...
        (void) barrel_inference_safe_model_free(m->model);
        m->model = NULL;
        m->release_pending = 0;
    }
...
static void model_drops_adapter(barrel_inference_model_t *m) {
    pthread_mutex_lock(&m->mu);
 ...
 *     `barrel_inference:unload/1` (which terminates the per-model
...
static void model_dtor(ErlNifEnv *env, void *obj) {
    (void) env;
    barrel_inference_model_t *m = (barrel_inference_model_t *) obj;
...
static void ctx_dtor(ErlNifEnv *env, void *obj) {
    ...
    if (c->ctx) {
        (void) barrel_inference_safe_free(c->ctx);
        c->ctx = NULL;
    }
    if (c->model_res) {
...
static void sampler_dtor(ErlNifEnv *env, void *obj) {
    (void) env;
    barrel_inference_sampler_t *s = (barrel_inference_sampler_t *) obj;
    if (s->chain) {
        (void) barrel_inference_safe_sampler_free(s->chain);
        s->chain = NULL;
    }
    if (s->ctx_res) {
...
static void adapter_dtor(ErlNifEnv *env, void *obj) {
    (void) env;
    barrel_inference_adapter_t *a = (barrel_inference_adapter_t *) obj;
    if (a->adapter) {
        barrel_inference_safe_adapter_lora_free(a->adapter);
        a->adapter = NULL;
    }
    if (a->model_res) {
...
static int load(ErlNifEnv *env, void **priv_data, ERL_NIF_TERM load_info) {
    ...
    if (barrel_inference_crc32c_init() != 0) {
        return -1;
    }
    /* llama_backend_init() is deferred to first model load via
     * barrel_inference_safe_backend_init_once(). NIF load only sets up
    ...
    MODEL_RT = enif_open_resource_type(
        env, NULL, "barrel_inference_model", model_dtor, ERL_NIF_RT_CREATE, NULL);
    if (!MODEL_RT) {
        return -1;
    }

    CTX_RT = enif_open_resource_type(
        env, NULL, "barrel_inference_context", ctx_dtor, ERL_NIF_RT_CREATE, NULL);
    if (!CTX_RT) {
        return -1;
    }

    ADAPTER_RT = enif_open_resource_type(
        env, NULL, "barrel_inference_adapter", adapter_dtor, ERL_NIF_RT_CREATE, NULL);
    if (!ADAPTER_RT) {
        return -1;
    }
...
static void unload(ErlNifEnv *env, void *priv_data) {
    ...
    barrel_inference_safe_log_unset();
}
...
typedef struct {
    const char *name;
    int value;
} barrel_inference_atom_enum_pair_t;
...
static int get_map_atom_enum(
    ...
    ERL_NIF_TERM map,
    const char *key,
    const barrel_inference_atom_enum_pair_t *table,
    size_t n,
    int *out
) {
    ERL_NIF_TERM v;
 ...
 * lives on `barrel_inference_model_t` so its lifetime spans the loaded
...
static const barrel_inference_atom_enum_pair_t SPLIT_MODE_TABLE[] = {
    {"none",  LLAMA_SPLIT_MODE_NONE},
    {"layer", LLAMA_SPLIT_MODE_LAYER},
    {"row",   LLAMA_SPLIT_MODE_ROW},
};

static const barrel_inference_atom_enum_pair_t FLASH_ATTN_TABLE[] = {
...
static const barrel_inference_atom_enum_pair_t GGML_TYPE_TABLE[] = {
    {"f16",  GGML_TYPE_F16},
    {"f32",  GGML_TYPE_F32},
    {"bf16", GGML_TYPE_BF16},
    {"q4_0", GGML_TYPE_Q4_0},
...
static ERL_NIF_TERM nif_crc32c(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    uint32_t crc = barrel_inference_crc32c_update(0, bin.data, bin.size);
 ...
 *
 * Used by barrel_inference_model to derive `vram_estimate_b` for `list_models`
...
static ERL_NIF_TERM nif_vram_info(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    (void) argc;
    (void) argv;
    if (barrel_inference_safe_backend_init_once() != 0) {
        return enif_make_tuple2(env, atom_error, atom_exception);
    }
    size_t n = barrel_inference_safe_backend_dev_count();
    size_t total_sum = 0, free_sum = 0;
    int found_non_cpu = 0;
    for (size_t i = 0; i < n; i++) {
        size_t f = 0, t = 0;
        int dt = 0;
        if (barrel_inference_safe_backend_dev_info(i, &f, &t, &dt) != 0) {
            continue;
        }
...
static ERL_NIF_TERM nif_load_model(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    barrel_inference_model_t *res = enif_alloc_resource(MODEL_RT, sizeof(*res));
    if (!res) {
    ...
    if (llama_max_devices() > (size_t) BARREL_INFERENCE_MAX_DEVICES) {
        enif_release_resource(res);
        return enif_make_tuple2(env, atom_error, atom_load_failed);
    }
    int ts_n = get_map_float_list(
        env, argv[1], "tensor_split",
        res->tensor_split, BARREL_INFERENCE_MAX_DEVICES
    );
    if (ts_n < 0) {
        enif_release_resource(res);
        return enif_make_badarg(env);
    ...
    barrel_inference_load_status_t status = BARREL_INFERENCE_LOAD_FAILED;
    struct llama_model *model =
        barrel_inference_safe_model_load_from_file_v2(path, params, &status);
    if (!model) {
        enif_release_resource(res);
        ERL_NIF_TERM why = (status == BARREL_INFERENCE_LOAD_MALFORMED)
                               ? atom_malformed_gguf
                               : atom_load_failed;
    ...
    if (pthread_mutex_init(&res->mu, NULL) != 0) {
        (void) barrel_inference_safe_model_free(model);
        enif_release_resource(res);
        return enif_make_tuple2(env, atom_error, atom_oom);
    }
    res->mu_inited = 1;
...
static ERL_NIF_TERM nif_free_model(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    int rc = barrel_inference_safe_model_free(to_free);
    m->model = NULL;
    m->release_pending = 0;
    pthread_mutex_unlock(&m->mu);
    if (rc != 0) {
...
static bool barrel_inference_decode_abort_cb(void *data) {
    barrel_inference_context_t *c = (barrel_inference_context_t *) data;
...
static void arm_decode(barrel_inference_context_t *c) {
...
static ERL_NIF_TERM classify_decode_error(ErlNifEnv *env, barrel_inference_context_t *c,
                                          int rc) {
    if (atomic_load_explicit(&c->abort_flag, memory_order_relaxed)) {
...
static ERL_NIF_TERM nif_new_context(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    if (get_map_uint(env, argv[1], "n_seq_max", &u)) {
        if (u > BARREL_INFERENCE_N_SEQ_MAX_CAP) {
            return enif_make_badarg(env);
        }
    ...
    struct llama_context *ctx = barrel_inference_safe_init_from_model(m->model, params);
    if (!ctx) {
        pthread_mutex_unlock(&m->mu);
        return enif_make_tuple2(env, atom_error, atom_context_failed);
    }
    barrel_inference_context_t *res = enif_alloc_resource(CTX_RT, sizeof(*res));
    if (!res) {
        (void) barrel_inference_safe_free(ctx);
        pthread_mutex_unlock(&m->mu);
        return enif_make_tuple2(env, atom_error, atom_oom);
    }
    memset(res, 0, sizeof(*res));
    ...
    for (size_t i = 0; i < (size_t) BARREL_INFERENCE_N_SEQ_MAX_CAP; i++) {
    ...
    barrel_inference_safe_set_abort_callback(ctx, barrel_inference_decode_abort_cb, res);
    m->active_contexts++;
...
static ERL_NIF_TERM nif_free_context(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    int free_rc = barrel_inference_safe_free(c->ctx);
    c->ctx = NULL;
    c->decode_ready = 0;
    barrel_inference_model_t *m = c->model_res;
    c->model_res = NULL;
    pthread_mutex_unlock(&c->mu);
    if (m) {
...
static ERL_NIF_TERM nif_tokenize(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    if (text.size > (size_t) BARREL_INFERENCE_MAX_TOKEN_TEXT) {
    ...
    const struct llama_vocab *vocab = barrel_inference_safe_model_get_vocab(m->model);
    if (!vocab) {
        pthread_mutex_unlock(&m->mu);
        return enif_make_tuple2(env, atom_error, atom_exception);
    }
     ...
     * BOS/EOS specials. Clamping it down to BARREL_INFERENCE_MAX_TOKENS would
     * force an unnecessary retry on inputs over ~1 byte/token average
     * and cap production workloads below the input text cap. The
     * input text cap (BARREL_INFERENCE_MAX_TOKEN_TEXT, enforced above) is
    ...
    int32_t n = barrel_inference_safe_tokenize(
    ...
    if (n < 0) {
        int32_t needed = -n;
        if (needed > BARREL_INFERENCE_MAX_TOKENS) {
            enif_free(tokens);
            pthread_mutex_unlock(&m->mu);
            return enif_make_tuple2(env, atom_error, atom_too_large);
        }
        ...
        n = barrel_inference_safe_tokenize(
            vocab, (const char *) text.data, text_len, tokens,
     ...
     * buffer larger than BARREL_INFERENCE_MAX_TOKENS (e.g. byte-fallback
    ...
    if (n > BARREL_INFERENCE_MAX_TOKENS) {
        enif_free(tokens);
        return enif_make_tuple2(env, atom_error, atom_too_large);
    }
...
static ERL_NIF_TERM nif_kv_pack(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    size_t need = barrel_inference_safe_state_seq_get_size(c->ctx, seq_id);
    if (need == SIZE_MAX) {
        pthread_mutex_unlock(&c->mu);
        return enif_make_tuple2(env, atom_error, atom_exception);
    }
    ...
    size_t written = barrel_inference_safe_state_seq_get_data(
...
static ERL_NIF_TERM nif_kv_unpack(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    size_t consumed = barrel_inference_safe_state_seq_set_data(
    ...
    if (seq_id < BARREL_INFERENCE_N_SEQ_MAX_CAP) {
        long pos_max = barrel_inference_safe_memory_seq_pos_max(c->ctx, seq_id);
...
static ERL_NIF_TERM nif_kv_seq_rm(ErlNifEnv *env, int argc,
    ...
    int rc = barrel_inference_safe_memory_seq_rm(c->ctx, seq_id, p0, p1);
     ...
     * to the seq's tail. */
    if (rc == 0 && seq_id < BARREL_INFERENCE_N_SEQ_MAX_CAP) {
        long pos_max = barrel_inference_safe_memory_seq_pos_max(c->ctx, seq_id);
        c->per_seq[seq_id].next_pos =
            pos_max < 0 ? 0 : (int32_t) (pos_max + 1);
        c->per_seq[seq_id].last_logits_idx = -1;
...
typedef struct {
    ...
    barrel_inference_sampler_t *sampler;    /* decode only */
    llama_token sampled;           /* decode only, set during pre-sample */
    int eog;                       /* decode only */
} barrel_inference_step_op_t;

static void free_step_ops(barrel_inference_step_op_t *ops, unsigned int n) {
    if (!ops) return;
...
static ERL_NIF_TERM nif_step(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    barrel_inference_step_op_t *ops = enif_alloc(sizeof(barrel_inference_step_op_t) * n_ops);
    if (!ops) {
        return enif_make_tuple2(env, atom_error, atom_oom);
    }
    memset(ops, 0, sizeof(barrel_inference_step_op_t) * n_ops);
    ...
    while (enif_get_list_cell(env, tail, &head, &tail)) {
        ...
        if (!enif_get_int(env, outer[0], &seq_id) ||
            seq_id < 0 || seq_id >= BARREL_INFERENCE_N_SEQ_MAX_CAP) {
            free_step_ops(ops, parsed);
            return enif_make_badarg(env);
        }
        ...
        if (strcmp(tag, "prefill") == 0) {
            ...
            barrel_inference_sampler_t *samp;
    ...
    const struct llama_model *model = barrel_inference_safe_get_model(c->ctx);
    const struct llama_vocab *vocab = model ? barrel_inference_safe_model_get_vocab(model) : NULL;
    ...
    for (unsigned int i = 0; i < n_ops; i++) {
        ...
        llama_token tok = barrel_inference_safe_sampler_sample(
        ...
        ops[i].sampled = tok;
        ops[i].eog = vocab ? barrel_inference_safe_vocab_is_eog(vocab, tok) : 0;
    }
     ...
     * shrink and retry. */
    uint32_t n_batch_cap = barrel_inference_safe_n_batch(c->ctx);
     ...
     * argument). */
    struct llama_batch batch = barrel_inference_safe_batch_init(total, 0, 1);
    if (!batch.token || !batch.pos || !batch.n_seq_id ||
        !batch.seq_id || !batch.logits) {
        barrel_inference_safe_batch_free(batch);
        pthread_mutex_unlock(&c->mu);
        free_step_ops(ops, n_ops);
        return enif_make_tuple2(env, atom_error, atom_oom);
    }
    ...
    arm_decode(c);
    int dr = barrel_inference_safe_decode(c->ctx, batch);
    if (dr != 0) {
        barrel_inference_safe_batch_free(batch);
        enif_free(last_logits_idx_new);
        c->decode_ready = 0;
        ERL_NIF_TERM why =
            (dr == BARREL_INFERENCE_DECODE_EXC_SENTINEL)
                ? atom_exception
...
static int read_token_list(ErlNifEnv *env, ERL_NIF_TERM list,
    ...
    if (n > (unsigned int) BARREL_INFERENCE_MAX_TOKENS) return -2;
    if (n == 0) {
        *out = NULL;
        *out_len = 0;
        return 1;
...
static ERL_NIF_TERM nif_prefill(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    const struct llama_vocab *vocab =
        model ? barrel_inference_safe_model_get_vocab(model) : NULL;
    int32_t n_vocab = vocab ? barrel_inference_safe_vocab_n_tokens(vocab) : 0;
    ...
    uint32_t n_ctx = barrel_inference_safe_n_ctx(c->ctx);
    uint32_t n_batch = barrel_inference_safe_n_batch(c->ctx);
    ...
    struct llama_batch batch = barrel_inference_safe_batch_get_one(tokens, n);
    int dr = barrel_inference_safe_decode(c->ctx, batch);
    if (dr == 0) c->decode_ready = 1;
    pthread_mutex_unlock(&c->mu);
    enif_free(tokens);
    if (dr == BARREL_INFERENCE_DECODE_EXC_SENTINEL) {
        return enif_make_tuple2(env, atom_error, atom_exception);
    }
    if (dr != 0) {
...
static ERL_NIF_TERM nif_decode_one(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
     ...
     * cached object. */
    llama_token tok = barrel_inference_safe_sampler_sample(c->smpl, c->ctx, -1);
    if (tok < 0) {
        pthread_mutex_unlock(&c->mu);
        return enif_make_tuple2(env, atom_error, atom_exception);
    }
    ...
    const struct llama_vocab *vocab =
        model ? barrel_inference_safe_model_get_vocab(model) : NULL;
    int eog = vocab ? barrel_inference_safe_vocab_is_eog(vocab, tok) : 0;

    llama_token tok_buf = tok;
    struct llama_batch batch = barrel_inference_safe_batch_get_one(&tok_buf, 1);
    arm_decode(c);
    int rc = barrel_inference_safe_decode(c->ctx, batch);
    if (rc == 0) c->decode_ready = 1;
    else c->decode_ready = 0;
    ERL_NIF_TERM why = atom_ok;
    if (rc != 0) {
        why = (rc == BARREL_INFERENCE_DECODE_EXC_SENTINEL) ? atom_exception
...
static ERL_NIF_TERM nif_detokenize(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    int32_t n_vocab = barrel_inference_safe_vocab_n_tokens(vocab);
    ...
    for (int32_t i = 0; i < n; i++) {
        ...
        char *grown = NULL;
        int32_t got = barrel_inference_safe_token_to_piece(
        ...
        if (got < 0) {
            ...
            piece_buf = grown;
            got = barrel_inference_safe_token_to_piece(
...
static ERL_NIF_TERM nif_apply_chat_template(ErlNifEnv *env, int argc,
    ...
    const char *tmpl = barrel_inference_safe_model_chat_template(m->model, NULL);
    if (!tmpl || tmpl[0] == '\0') {
        pthread_mutex_unlock(&m->mu);
        free_chat_msgs(msgs, n_msgs);
        enif_free(msgs);
    ...
    int32_t written = barrel_inference_safe_chat_apply_template(
     ...
     * `barrel_inference_safe_chat_apply_template`.) */
    if (written > 0 && written > buf_size) {
        if (written > (int32_t) BARREL_INFERENCE_MAX_TOKEN_TEXT) {
            pthread_mutex_unlock(&m->mu);
            free_chat_msgs(msgs, n_msgs);
            enif_free(msgs);
            enif_free(buf);
        ...
        written = barrel_inference_safe_chat_apply_template(
            tmpl, msgs, (size_t) n_msgs, true, buf, buf_size);
    ...
    int32_t n = barrel_inference_safe_tokenize(vocab, buf, written, tokens, n_max,
                                      true, true);
    if (n < 0 && n != INT32_MIN) {
        int32_t needed = -n;
        if (needed > BARREL_INFERENCE_MAX_TOKENS) {
        ...
        n = barrel_inference_safe_tokenize(vocab, buf, written, tokens, needed,
                                  true, true);
    }
    pthread_mutex_unlock(&m->mu);
    free_chat_msgs(msgs, n_msgs);
...
static ERL_NIF_TERM nif_embed(ErlNifEnv *env, int argc,
    ...
    int32_t n_embd = barrel_inference_safe_n_embd(model);
    if (n_embd <= 0) {
        pthread_mutex_unlock(&c->mu);
        enif_free(tokens);
    ...
    if (barrel_inference_safe_set_embeddings(c->ctx, true) != 0) {
        pthread_mutex_unlock(&c->mu);
        enif_free(tokens);
        return enif_make_tuple2(env, atom_error, atom_exception);
    }
    ...
    float *embd = barrel_inference_safe_get_embeddings_seq(c->ctx, 0);
    if (!embd) {
        embd = barrel_inference_safe_get_embeddings(c->ctx);
    }
    if (!embd) {
        pthread_mutex_unlock(&c->mu);
        enif_free(tokens);
 ...
 * by barrel_inference:verify/4. Builds a custom batch with logits[i]=1 on
...
static ERL_NIF_TERM nif_forward_with_argmax(ErlNifEnv *env, int argc,
    ...
    long pos_max = barrel_inference_safe_memory_seq_pos_max(c->ctx, 0);
    if (pos_max == -2) {
        pthread_mutex_unlock(&c->mu);
        enif_free(tokens);
        return enif_make_tuple2(env, atom_error, atom_exception);
    ...
    int frc = barrel_inference_safe_forward_with_argmax(
    ...
    for (int32_t i = n - 1; i >= 0; i--) {
        ...
        if (barrel_inference_safe_vocab_is_eog(vocab, out[i])) {
            tok_term = atom_eos;
        } else {
...
static struct llama_sampler *build_default_greedy_chain(void) {
    ...
    struct llama_sampler *chain = barrel_inference_safe_sampler_chain_init(sp);
    if (!chain) return NULL;
    struct llama_sampler *greedy = barrel_inference_safe_sampler_init_greedy();
    if (!greedy) {
        (void) barrel_inference_safe_sampler_free(chain);
        return NULL;
    }
    if (barrel_inference_safe_sampler_chain_add(chain, greedy) != 0) {
        (void) barrel_inference_safe_sampler_free(greedy);
        (void) barrel_inference_safe_sampler_free(chain);
        return NULL;
    }
    return chain;
...
static int chain_append(struct llama_sampler *chain,
    ...
    if (!stage) return -1;
    if (barrel_inference_safe_sampler_chain_add(chain, stage) != 0) {
        (void) barrel_inference_safe_sampler_free(stage);
        return -1;
    }
    return 0;
}
...
build_sampler_chain_from_map(ErlNifEnv *env, ERL_NIF_TERM cfg,
    ...
    if (has_grammar) {
        const struct llama_model *model = barrel_inference_safe_get_model(ctx);
        const struct llama_vocab *vocab =
            model ? barrel_inference_safe_model_get_vocab(model) : NULL;
        if (!vocab) {
            (void) barrel_inference_safe_sampler_free(chain);
    ...
    if (has_rep && rep_val != 1.0) {
        if (chain_append(chain,
                         barrel_inference_safe_sampler_init_penalties(
    ...
    if (has_top_k && top_k_val > 0) {
        if (chain_append(chain,
                         barrel_inference_safe_sampler_init_top_k(top_k_val)) != 0) {
            (void) barrel_inference_safe_sampler_free(chain);
            *out_err_atom = atom_oom;
            return NULL;
        }
    ...
    if (has_top_p && top_p_val < 1.0) {
        if (chain_append(chain,
                         barrel_inference_safe_sampler_init_top_p((float) top_p_val,
                                                         1)) != 0) {
            (void) barrel_inference_safe_sampler_free(chain);
            *out_err_atom = atom_oom;
            return NULL;
    ...
    if (has_min_p && min_p_val > 0.0) {
        if (chain_append(chain,
                         barrel_inference_safe_sampler_init_min_p((float) min_p_val,
                                                         1)) != 0) {
            (void) barrel_inference_safe_sampler_free(chain);
            *out_err_atom = atom_oom;
            return NULL;
    ...
    if (has_temp && temp_val > 0.0) {
        if (chain_append(chain,
                         barrel_inference_safe_sampler_init_temp((float) temp_val)) != 0) {
            (void) barrel_inference_safe_sampler_free(chain);
            *out_err_atom = atom_oom;
            return NULL;
        }
        if (chain_append(chain,
                         barrel_inference_safe_sampler_init_dist(seed_val)) != 0) {
            (void) barrel_inference_safe_sampler_free(chain);
            *out_err_atom = atom_oom;
            return NULL;
        }
        ...
        if (chain_append(chain, barrel_inference_safe_sampler_init_greedy()) != 0) {
            (void) barrel_inference_safe_sampler_free(chain);
            *out_err_atom = atom_oom;
            return NULL;
        }
...
static ERL_NIF_TERM nif_adapter_load(ErlNifEnv *env, int argc,
    ...
    struct llama_adapter_lora *adapter =
        barrel_inference_safe_adapter_lora_init(m->model, path);
    if (!adapter) {
        pthread_mutex_unlock(&m->mu);
        return enif_make_tuple2(env, atom_error, atom_load_failed);
    }
    barrel_inference_adapter_t *res =
        enif_alloc_resource(ADAPTER_RT, sizeof(*res));
    if (!res) {
        barrel_inference_safe_adapter_lora_free(adapter);
        pthread_mutex_unlock(&m->mu);
        return enif_make_tuple2(env, atom_error, atom_oom);
    }
    memset(res, 0, sizeof(*res));
...
typedef struct {
    barrel_inference_adapter_t *w;
    float scale;
    unsigned orig_idx;
} adapter_entry_t;

static int adapter_entry_cmp(const void *a, const void *b) {
    barrel_inference_adapter_t *aw = ((const adapter_entry_t *) a)->w;
    barrel_inference_adapter_t *bw = ((const adapter_entry_t *) b)->w;
    if (aw < bw) return -1;
    if (aw > bw) return 1;
    return 0;
}
...
static ERL_NIF_TERM nif_set_adapters(ErlNifEnv *env, int argc,
    ...
    rc = barrel_inference_safe_set_adapters_lora(c->ctx, adapters, n, scales);
    pthread_mutex_unlock(&c->mu);
...
static ERL_NIF_TERM nif_sampler_new(ErlNifEnv *env, int argc,
    ...
    barrel_inference_sampler_t *res = enif_alloc_resource(SAMPLER_RT, sizeof(*res));
    if (!res) {
        (void) barrel_inference_safe_sampler_free(chain);
        return enif_make_tuple2(env, atom_error, atom_oom);
    }
