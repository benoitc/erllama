...
-record(state, {
    ...
    pinned :: #{binary() => erllama_cache:cache_key()},
    ...
    live_key = undefined :: erllama_cache:cache_key() | undefined,
...
probe_text_prefix(Fp, QT, CtxHash, Bytes, [Len | Rest], Probes) ->
    Candidate = erllama_cache_key:make_text(
...
`Ns` (`erllama_cache_key:namespace/3`). The pinned row is
...
""".
-spec pin_row(binary(), erllama_cache:cache_key()) -> ok.
...
""".
-spec set_live_key(erllama_cache:cache_key() | undefined) -> ok.
