...
%%
-module(barrel_inference_cache_kvc).
-moduledoc """
...
[48..71]   Trailer (24 bytes, barrel_inference-specific):
              u64    payload_offset
              u64    payload_length
              u32    payload_crc32c
              u32    reserved
...
0x03 quant_type         1 byte (cache-internal byte; see barrel_inference_cache_key)
0x04 ctx_params_hash    32 bytes
0x05 hostname           variable
0x06 barrel_inference_version    variable
...
-type build_meta() :: #{
    ...
    prompt_text => binary(),
    hostname => binary(),
    barrel_inference_version => binary(),
...
-define(TLV_BARREL_INFERENCE_VERSION, 16#06).
...
do_build(Meta, Payload) ->
    ...
    PayloadCrc = barrel_inference_nif:crc32c(Payload),
...
build_tlv(Meta, Tokens) ->
    QuantTypeByte = barrel_inference_cache_key:quant_byte(maps:get(quant_type, Meta)),
    ...
    Required = [
        ...
        {?TLV_TOKEN_IDS, barrel_inference_cache_key:encode_tokens(Tokens)}
    ],
    Optional = [
        {?TLV_HOSTNAME, default_hostname(Meta)},
        {?TLV_BARREL_INFERENCE_VERSION, default_barrel_inference_version(Meta)},
...
default_barrel_inference_version(Meta) ->
    case maps:get(barrel_inference_version, Meta, undefined) of
        undefined ->
...
-spec parse(binary(), barrel_inference_cache_key:key()) ->
...
verify(Info, Payload, ExpectedCrc, ExpectedKey) ->
    ActualCrc = barrel_inference_nif:crc32c(Payload),
    case ActualCrc =:= ExpectedCrc of
        false ->
            {error, {crc_mismatch, ExpectedCrc, ActualCrc}};
        true ->
            ActualKey = barrel_inference_cache_key:make(#{
...
store_tlv(?TLV_QUANT_TYPE, <<B:8>>, Acc) ->
    case barrel_inference_cache_key:quant_atom(B) of
...
store_tlv(?TLV_HOSTNAME, V, Acc) ->
    Acc#{hostname => V};
store_tlv(?TLV_BARREL_INFERENCE_VERSION, V, Acc) ->
    Acc#{barrel_inference_version => V};
...
store_tlv(?TLV_TOKEN_IDS, V, Acc) when byte_size(V) rem 4 =:= 0 ->
    Acc#{tokens => barrel_inference_cache_key:decode_tokens(V)};
