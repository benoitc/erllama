...
%%
%% @doc
%% Single NIF entry module for barrel_inference.
%%
...
init() ->
    PrivDir =
        case code:priv_dir(barrel_inference) of
            {error, bad_name} ->
    ...
    SoName = filename:join(PrivDir, "barrel_inference_nif"),
...
%% tokens). Used by barrel_inference:verify/4 for speculative-decoding
