...
%%
-module(erllama_model_evictor).
-moduledoc """
Behaviour for proactive model eviction under memory pressure.

`erllama_scheduler` is the engine-level memory-pressure responder, but
...
%% `erllama_pressure:sample/1`. Normalises the return - not just
