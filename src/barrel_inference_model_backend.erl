...
%%
-module(barrel_inference_model_backend).
-moduledoc """
Behaviour describing the operations the `barrel_inference_model` gen_statem
needs from a backing inference engine.

Two backends ship in v0.2:

  `barrel_inference_model_stub` — deterministic phash2-based stubs; used
      by tests that don't have a GGUF on disk.
  `barrel_inference_model_llama` — real llama.cpp via the NIF.
...
-callback tokenize(state(), Text :: binary()) ->
    [barrel_inference_nif:token_id()] | {error, term()}.

-callback detokenize(state(), [barrel_inference_nif:token_id()]) ->
    binary() | {error, term()}.

-callback prefill(state(), [barrel_inference_nif:token_id()]) -> ok | {error, term()}.

-callback decode_one(state(), ContextTokens :: [barrel_inference_nif:token_id()]) ->
    {ok, barrel_inference_nif:token_id()}
    | {eog, barrel_inference_nif:token_id()}
    | {error, term()}.

-callback kv_pack(state(), Tokens :: [barrel_inference_nif:token_id()]) ->
    binary() | {error, term()}.
...
-callback kv_pack(state(), Tokens :: [barrel_inference_nif:token_id()], seq_id()) ->
    binary() | {error, term()}.
...
%% stub) can omit this; `barrel_inference_model` checks `is_exported/3` and
...
-callback embed(state(), [barrel_inference_nif:token_id()]) ->
    {ok, [float()]} | {error, term()}.
...
%% Called by `barrel_inference_model` immediately before the first decode_one
...
%% `barrel_inference_nif:configure_sampler/2` for the precise semantics.
...
%% Backend-specific metadata used by barrel_inference_model:list_models/0
%% beyond what the gen_statem already tracks. The default backend
%% (barrel_inference_model_llama) returns model byte size, total layer
%% count, and the n_gpu_layers value the user passed at load
%% time. barrel_inference_model uses these to compute `vram_estimate_b`.
...
-callback verify(
    state(),
    PrefixTokens :: [barrel_inference_nif:token_id()],
    Candidates :: [barrel_inference_nif:token_id()],
    K :: pos_integer()
) ->
    {ok, AcceptedCount :: non_neg_integer(), NextToken :: barrel_inference_nif:token_id() | eos,
        NewState :: state()}
    | {error, term()}.
...
%% Opaque handle the engine can hand to `barrel_inference_nif:request_abort/1`
...
-type step_op() ::
    {prefill, [barrel_inference_nif:token_id()]}
    | {decode, sampler_ref()}.
-type step_result() ::
    prefilled
    | {token, barrel_inference_nif:token_id(), 0 | 1}
    %% Thinking-phase token: scheduler detokenises and emits
    %% {barrel_inference_token, Ref, {thinking_delta, Bin}} instead of a plain
    %% text fragment. Backends without extended-thinking support
    %% never emit this variant.
    | {thinking_token, barrel_inference_nif:token_id()}
...
%% emitting the corresponding `barrel_inference_thinking_end` message.
