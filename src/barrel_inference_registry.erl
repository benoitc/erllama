...
%%
-module(barrel_inference_registry).
-moduledoc """
ETS-backed `via` callback for naming `barrel_inference_model` gen_statems by
binary `model_id()`.
...
```
gen_statem:start_link({via, barrel_inference_registry, ModelId}, ...)
```
...
%% barrel_inference:list_models/0.
