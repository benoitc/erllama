...
-record(req, {
    ...
    cold_save_segments =
        undefined :: undefined | [{erllama_cache_kvc:save_reason(), [non_neg_integer()]}],
...
resolve_policy(Config, NBatch) ->
    Defaults = #{
        ...
        session_resume_wait_ms => application:get_env(
            erllama, session_resume_wait_ms, 500
        ),
...
setup_cold(Req, Data) ->
    ...
    PlainSegs = erllama_cache_policy:cold_save_segments(Tokens, Data#data.policy),
...
handle_common(_State, {call, From}, chat_purge, Data) ->
    erllama_chat_cache:purge(Data#data.model_id),
    reply(From, ok, Data);
...
fire_agent_prefix_save(Ctx, Req, Data) ->
    ...
    case fire_save_if(Should, agent_prefix, Ctx, Req, Data) of
        fired ->
            ...
            Ns = erllama_cache_key:namespace(
                request_fp(Req, Data), Data#data.quant_type, Data#data.ctx_params_hash
            ),
            ok = erllama_cache_meta_srv:pin_row(Ns, Key),
    ...
    Req1 =
        case FinishKey of
            ...
            _ ->
                ...
                erllama_cache_meta_srv:set_live_key(FinishKey),
...
try_longest_prefix(PromptTokens, PromptBytes, Req, Data) ->
    ...
    case erllama_cache_meta_srv:lookup_longest_text_prefix(KeyMeta, PromptBytes) of
        {ok, MatchBytes, Row} ->
            resume_at_prefix(
