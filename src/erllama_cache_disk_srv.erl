...
scan_entry_row(Bin, Info) ->
    ...
    TokensBin = erllama_cache_key:encode_tokens(maps:get(tokens, Info)),
    ...
    PinNs =
        case maps:get(save_reason, Info, undefined) of
            agent_prefix -> erllama_cache_key:namespace(Fp, QuantType, CtxHash);
            _ -> undefined
        end,
...
register_existing(Tier, Root) ->
    ...
    lists:foreach(
        fun({Key, Header, Size, TokensBin, TextBytes, PinNs}) ->
            ...
            case PinNs of
                undefined -> ok;
                Ns -> ok = erllama_cache_meta_srv:pin_row(Ns, Key)
            end
        end,
        Entries
    ).
