...
lives in `barrel_inference_cache_meta_srv` and `barrel_inference_cache_writer`; this
...
get_counters() ->
    barrel_inference_cache_counters:snapshot().
...
gc() ->
    barrel_inference_cache_meta_srv:gc().
...
evict_bytes(TargetBytes) ->
    barrel_inference_cache_meta_srv:evict_bytes(TargetBytes).
