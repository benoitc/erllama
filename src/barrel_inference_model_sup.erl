...
%%
-module(barrel_inference_model_sup).
-moduledoc """
Dynamic supervisor for `barrel_inference_model` gen_statems. Each loaded
...
registers itself in `barrel_inference_registry` under that key. Returns the
started pid.
""".
...
init([]) ->
    ...
    Child = #{
        ...
        start => {barrel_inference_model, start_link, []},
        restart => temporary,
        shutdown => 5000,
        type => worker,
        modules => [barrel_inference_model]
    },
    {ok, {SupFlags, [Child]}}.

pid_of(Pid) when is_pid(Pid) -> Pid;
pid_of(ModelId) when is_binary(ModelId) -> barrel_inference_registry:whereis_name(ModelId).
