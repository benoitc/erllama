...
prefixes. See `erllama_chat:parse/3' for the returned map
shape.
""".
-spec chat_parse(
    erllama_nif:chat_params_ref(), binary(), boolean()
) ->
    {ok, map()} | {error, term()}.
...
list_cached_prefixes(ModelId, PromptTokens) when is_binary(ModelId), is_list(PromptTokens) ->
    case erllama_registry:whereis_name(ModelId) of
        ...
        _Pid ->
            ...
            case erllama_model:detokenize(ModelId, PromptTokens) of
                {ok, PromptBytes} ->
                    case
                        erllama_cache:lookup_longest_text_prefix(
                            KeyMeta, PromptBytes
                        )
                    of
