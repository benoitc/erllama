...
%%
-module(barrel_inference_cache_ram).
-moduledoc """
RAM tier slab store.

Owns the `barrel_inference_cache_ram_slabs` ETS table. Reads and deletes
...
-define(SLABS, barrel_inference_cache_ram_slabs).
...
-spec put(barrel_inference_cache:cache_key(), binary(), binary()) -> ok.
...
-spec load(barrel_inference_cache:cache_key()) -> {ok, binary()} | miss.
load(Key) ->
...
-spec delete(barrel_inference_cache:cache_key()) -> ok.
delete(Key) ->
