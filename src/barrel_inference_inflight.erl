...
%%
-module(barrel_inference_inflight).
-moduledoc """
Tracks in-flight streaming inference requests.

Each `barrel_inference:infer/4` admit produces a unique `reference()` and an
entry in a public ETS table mapping that reference back to the
serving `barrel_inference_model` gen_statem. `barrel_inference:cancel/1` looks up the
...
-define(TABLE, ?MODULE).
-define(OBS_TABLE, barrel_inference_model_obs).
-define(ABORT_TABLE, barrel_inference_model_abort).
...
obs_put(ModelId, Row) when is_binary(ModelId), is_tuple(Row) ->
    %% Tolerant of a missing table: when `barrel_inference_inflight` has not
...
%% handle (the backend's context ref) that `barrel_inference:cancel/1` can pass
%% to `barrel_inference_nif:request_abort/1` to interrupt an in-flight decode
...
init([]) ->
    ...
    %% (including remote nodes via erpc, used by barrel_inference_cluster)
