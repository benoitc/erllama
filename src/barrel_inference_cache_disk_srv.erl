...
%%
-module(barrel_inference_cache_disk_srv).
-moduledoc """
...
Save pipeline (`save/3`):
  ...
  1. Build framed bytes via `barrel_inference_cache_kvc:build/2`.
  ...
  5. `prim_file:make_link/2` to publish at `<hex>.kvc`. EEXIST is
     ...
     file and retry once.
  6. `barrel_inference_nif:fsync_dir/1` on the root.
...
-type write_out() :: #{
    key := barrel_inference_cache:cache_key(),
    tmp := file:name(),
    final := file:name(),
    prefix := binary(),
    payload := binary(),
...
-spec write_tmp(file:name(), barrel_inference_cache_kvc:build_meta(), binary()) ->
...
write_tmp(Root, BuildMeta, Payload) ->
    ...
    case barrel_inference_cache_kvc:build(BuildMeta, Payload) of
        {ok, Prefix} ->
...
-spec publish(file:name(), file:name(), file:name(), barrel_inference_cache:cache_key()) ->
    {ok, barrel_inference_cache:cache_key(), binary(), non_neg_integer()}
    | {error, term()}.
...
handle_eexist(Key, Prefix, Payload, TmpPath, FinalPath, Root) ->
    case validate_at(FinalPath, Key) of
        {ok, _Info, _Payload} ->
            ...
            ok = barrel_inference_nif:fsync_dir(unicode_path(Root)),
...
finalise(Key, Prefix, _Payload, FinalPath, Root) ->
    case barrel_inference_nif:fsync_dir(unicode_path(Root)) of
        ok ->
            case validate_at(FinalPath, Key) of
...
parse_or_drop(Bin, Key, Path) ->
    case barrel_inference_cache_kvc:parse(Bin, Key) of
        ...
        {error, R} ->
            ...
            barrel_inference_cache_counters:incr(?C_CORRUPT_FILES),
            {error, R}
    end.
...
validate_at(Path, Key) ->
    case file:read_file(Path) of
        {ok, Bin} -> barrel_inference_cache_kvc:parse(Bin, Key);
        {error, _} = E -> E
    end.
