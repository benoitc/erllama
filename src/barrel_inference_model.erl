...
binary off to `barrel_inference_cache_writer`.
...
-type model() :: barrel_inference_registry:model_id() | pid().
-type model_info() :: #{
    id := binary(),
...
-type infer_params() :: #{
    ...
    %% `{barrel_inference_token, Ref, {thinking_delta, Bin}}`. When the
    %% thinking phase closes the scheduler emits a single
    %% `{barrel_inference_thinking_end, Ref, Sig}` before any non-thinking
    ...
    %% request, it synthesises the `barrel_inference_thinking_end` close
...
-record(req, {
    ...
    %% `barrel_inference_error` (e.g. backend contract violation). The
    %% finish path skips the `barrel_inference_done` send in that case so
...
-record(data, {
    ...
    quant_type :: barrel_inference_cache_key:quant_type(),
    quant_bits :: non_neg_integer(),
    ctx_params_hash :: <<_:256>>,
    context_size :: non_neg_integer(),
    policy :: barrel_inference_cache_policy:config(),
    %% Inference backend: barrel_inference_model_stub | barrel_inference_model_llama.
    backend :: module(),
    backend_state :: term(),
    ...
    %% `get_last_sampler_cfg/1`.
    last_sampler_cfg = undefined :: map() | undefined,
    %% Test-visible holdover so barrel_inference_sampler_tests can inspect
...
start_link(ModelId, Config) when is_binary(ModelId) ->
    gen_statem:start_link(
        {via, barrel_inference_registry, ModelId}, ?MODULE, [ModelId, Config], []
    ).
...
- `{barrel_inference_token, Ref, binary()}` per generated token (text fragment;
  suppressed when the detokenized binary is empty)
- `{barrel_inference_token_id, Ref, integer()}` per generated token (always
  delivered, including for tokens whose text fragment is empty;
  used by speculative-decoding collectors)
- `{barrel_inference_done, Ref, stats()}` on normal completion
- `{barrel_inference_error, Ref, term()}` on failure
...
inter-token boundary; the model emits a final `{barrel_inference_done, Ref,
Stats}` with `cancelled => true` after the running decode step
completes.
""".
-spec cancel(reference()) -> ok.
cancel(Ref) when is_reference(Ref) ->
    case barrel_inference_inflight:lookup(Ref) of
        {ok, ModelPid} ->
...
`barrel_inference_scheduler` (future) when GPU memory pressure requires this
...
%% by barrel_inference_sampler_tests to verify the cfg projection without
...
init([ModelId, Config]) ->
    Backend = maps:get(backend, Config, barrel_inference_model_stub),
...
build_init_data(ModelId, Config, Backend, BState) ->
    ...
    #data{
        model_id = ModelId,
        tier_srv = maps:get(tier_srv, Config, barrel_inference_cache_ram),
...
%% fall back to the app env defaults declared in `barrel_inference.app.src`.
%%
...
resolve_policy(Config, NBatch) ->
    Defaults = #{
        min_tokens => application:get_env(barrel_inference, min_tokens, 512),
        cold_min_tokens => application:get_env(barrel_inference, cold_min_tokens, 512),
        cold_max_tokens => application:get_env(barrel_inference, cold_max_tokens, 30000),
        continued_interval => application:get_env(barrel_inference, continued_interval, 2048),
        boundary_trim_tokens => application:get_env(barrel_inference, boundary_trim_tokens, 32),
        boundary_align_tokens => application:get_env(barrel_inference, boundary_align_tokens, 2048),
        session_resume_wait_ms => application:get_env(
            barrel_inference, session_resume_wait_ms, 500
        ),
...
terminate(_Reason, _State, #data{model_id = ModelId, backend = B, backend_state = S}) ->
    _ = barrel_inference_inflight:obs_delete(ModelId),
    _ = barrel_inference_inflight:delete_abort_handle(self()),
    B:terminate(S),
    ok;
...
publish_abort_handle(#data{backend = Mod, backend_state = S}) ->
    ...
    case Handle of
        {ok, H} ->
            _ = barrel_inference_inflight:set_abort_handle(self(), H),
            ok;
        undefined ->
            _ = barrel_inference_inflight:delete_abort_handle(self()),
            ok
...
finalize_recompute(Data, Result) ->
    ...
    NewFp = barrel_inference_cache_key:effective_fingerprint(
...
build_model_info(State, Data) ->
    ...
    %% lock-free accessors (`barrel_inference:last_cache_hit/1`, etc.) agree
    ...
    LastHit =
        case barrel_inference_inflight:obs_get(Data#data.model_id) of
    ...
    #{
        ...
        %% by `barrel_inference:phase/1` and the cluster router.
        phase => State,
        ...
        quant_tag => barrel_inference_cache_key:quant_tag(
...
obs_install_initial(Data) ->
    ...
    _ = barrel_inference_inflight:obs_put(Data#data.model_id, Row),
    ok.

obs_refresh(Phase, Data) ->
    _ = barrel_inference_inflight:obs_put(Data#data.model_id, obs_row(Phase, Data)),
    ok.
...
%% {barrel_inference_token, Ref, {thinking_delta, Bin}} to streaming callers
...
%% backend bug -- the request is failed via barrel_inference_error.
...
->
    ...
    case Bin of
        <<>> ->
            ...
            Req1;
        _ ->
            Pid ! {barrel_inference_token, Ref, {thinking_delta, Bin}},
...
%% synthesise the `barrel_inference_thinking_end` close immediately so the
...
->
    ...
    Pid ! {barrel_inference_thinking_end, Ref, Sig},
...
->
    barrel_inference_inflight:unregister(Ref),
    Pid ! {barrel_inference_error, Ref, thinking_not_enabled},
    ok;
...
maybe_fire_continued_for_req(Req, Data) ->
    ...
    Should = barrel_inference_cache_policy:should_continued_save(
...
->
    barrel_inference_inflight:unregister(Ref),
    Pid ! {barrel_inference_done, Ref, Stats},
    ok;
...
%% `{barrel_inference_token, _, _}` message. Only relevant for streaming
...
->
    Pid ! {barrel_inference_token, Ref, Tail},
    ok;
...
try_session_resume(PromptTokens, PromptBytes, ParentKey, Req, Data) ->
    ...
    case barrel_inference_cache_meta_srv:lookup_exact_or_wait(ParentKey, Wait) of
        {ok, _Row} ->
...
pin_and_load(Key, SeqId, Data) ->
    ...
    Result =
        case barrel_inference_cache_meta_srv:checkout(Key, self()) of
                    ...
                    case Bin of
                        ...
                        _ ->
                            ...
                            Tokens =
                                case TokensBin of
                                    undefined -> [];
                                    _ -> barrel_inference_cache_key:decode_tokens(TokensBin)
                                end,
                            {ok, Tokens}
                    end
                after
                    ok = barrel_inference_cache_meta_srv:checkin(HolderRef)
                end;
            {error, busy} ->
                miss;
            miss ->
    ...
    barrel_inference_cache_counters:add(?C_LOAD_TOTAL_NS, max(Elapsed, 0)),
    Result.

load_payload(ram, _Loc, Key, _Data) ->
    case barrel_inference_cache_ram:load(Key) of
        {ok, B} -> B;
        miss -> <<>>
    end;
load_payload(_Tier, _Loc, Key, Data) ->
    case barrel_inference_cache_disk_srv:load(Data#data.tier_srv, Key) of
...
fire_finish_save_for_req(LiveTokens, Req, Data) ->
    Should = barrel_inference_cache_policy:should_finish_save(
...
fire_save_if(true, Reason, Tokens, Req, Data) ->
    ...
    Elapsed = erlang:monotonic_time(nanosecond) - T0,
    barrel_inference_cache_counters:add(?C_PACK_TOTAL_NS, max(Elapsed, 0)),
    case
        barrel_inference_cache_writer:save(
    ...
    of
        ...
        {error, already_present} ->
            ...
            fired;
        {error, _SaveErr} ->
            barrel_inference_cache_counters:incr(?C_SAVES_DROPPED),
            not_fired
    end.
...
%% Crashes with `{noproc, {barrel_inference_model, not_found, ModelId}}` if
