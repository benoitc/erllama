...
%%
-module(barrel_inference_pressure).
-moduledoc """
Behaviour and helpers for memory-pressure samplers used by
`barrel_inference_scheduler`. A sampler is a stateless module that returns
...
sample(noop) ->
    {0, 1};
sample(system) ->
    barrel_inference_pressure_system:sample();
sample(nvidia_smi) ->
    barrel_inference_pressure_nvidia_smi:sample();
