...
%%
-module(barrel_inference_cache_writer).
-moduledoc """
...
-define(TABLE, barrel_inference_cache_writer_sem).
...
-spec save(
    atom(),
    disk | ram_file,
    barrel_inference_cache_kvc:build_meta(),
    binary()
) -> {ok, barrel_inference_cache:cache_key()} | {error, term()}.
...
default_max_concurrent() ->
    case application:get_env(barrel_inference, writer_max_concurrent) of
...
do_save(TierSrv, Tier, BuildMeta, Payload) ->
    case derive_key(BuildMeta) of
        {ok, Key} ->
            case barrel_inference_cache_meta_srv:reserve_save(Key, Tier, self()) of
                {ok, Token} ->
                    do_save_reserved(TierSrv, Key, Token, BuildMeta, Payload);
                {error, already_present} ->
                    barrel_inference_cache_counters:incr(?C_DUPLICATE_DROPPED),
                    {error, already_present};
                {error, _} = E ->
                    E
            end;
...
do_save_reserved(TierSrv, Key, Token, BuildMeta, Payload) ->
    Root = barrel_inference_cache_disk_srv:dir(TierSrv),
    case barrel_inference_cache_disk_srv:write_tmp(Root, BuildMeta, Payload) of
        {ok, WriteOut} ->
            after_write_tmp(Key, Token, BuildMeta, WriteOut);
        {error, _} = E ->
            _ = barrel_inference_cache_meta_srv:cancel_reservation(Key, Token),
            E
    end.

after_write_tmp(Key, Token, BuildMeta, WriteOut) ->
    case barrel_inference_cache_meta_srv:check_reservation(Key, Token) of
        ok ->
            after_check_reservation(Key, Token, BuildMeta, WriteOut);
        {error, expired} = E ->
            barrel_inference_cache_disk_srv:abort_tmp(WriteOut),
            E
    end.

after_check_reservation(Key, Token, BuildMeta, WriteOut) ->
    case barrel_inference_cache_disk_srv:publish(WriteOut) of
...
bump_save_counter(cold) -> barrel_inference_cache_counters:incr(?C_SAVES_COLD);
bump_save_counter(continued) -> barrel_inference_cache_counters:incr(?C_SAVES_CONTINUED);
