...
%%
-module(barrel_inference_model_llama).
-moduledoc """
Real-llama.cpp backend for `barrel_inference_model`.

Owns a `model_ref` and a `context_ref` from `barrel_inference_nif`. The
gen_statem hands its decode/kv operations through this module;
this module forwards to the NIF.

Config (passed through `barrel_inference_model:start_link/2`):
  model_path :: file:name() | binary()  (required)
  model_opts :: map()  (forwarded to barrel_inference_nif:load_model/2)
  context_opts :: map()  (forwarded to barrel_inference_nif:new_context/2)

`model_opts` and `context_opts` flow through to the NIF unchanged.
See `barrel_inference_nif:load_model/2` and `barrel_inference_nif:new_context/2` for
...
-record(s, {
    ...
    ctx :: barrel_inference_nif:context_ref(),
    %% Context options passed to barrel_inference_nif:new_context/2. Retained
...
init(Config) ->
    ...
    case barrel_inference_nif:load_model(Path, MOpts) of
...
open_context(Model, Config, MOpts) ->
    ...
    case barrel_inference_nif:new_context(Model, COpts) of
        {ok, Ctx} ->
...
build_state(Model, Ctx, Config, MOpts) ->
    ...
    #s{
        ...
        model_size_bytes = safe_uint(barrel_inference_nif:model_size(Model)),
...
tokenize_marker(Model, Bin) when is_binary(Bin) ->
    case
        barrel_inference_nif:tokenize(Model, Bin, #{add_special => false, parse_special => true})
    of
...
reset_context(#s{ctx = OldCtx, model = Model, context_opts = COpts} = S) ->
    _ = barrel_inference_nif:free_context(OldCtx),
    case barrel_inference_nif:new_context(Model, COpts) of
...
prefill(#s{ctx = C}, Tokens) ->
    barrel_inference_nif:prefill(C, Tokens).

decode_one(#s{ctx = C}, _ContextTokens) ->
    barrel_inference_nif:decode_one(C).

kv_pack(#s{ctx = C}, Tokens) ->
    barrel_inference_nif:kv_pack(C, Tokens, length(Tokens)).
...
kv_pack(#s{ctx = C}, Tokens, SeqId) when is_integer(SeqId), SeqId >= 0 ->
    barrel_inference_nif:kv_pack(C, Tokens, length(Tokens), SeqId).

kv_unpack(#s{ctx = C}, Bin) ->
    barrel_inference_nif:kv_unpack(C, Bin, 0).

kv_unpack(#s{ctx = C}, Bin, SeqId) when is_integer(SeqId), SeqId >= 0 ->
    barrel_inference_nif:kv_unpack(C, Bin, SeqId).
...
seq_rm(#s{ctx = C}, SeqId) when is_integer(SeqId), SeqId >= 0 ->
    barrel_inference_nif:kv_seq_rm(C, SeqId, 0, -1).

%% Wipe seq 0 entirely. p0=0, p1=-1 means "from position 0 to infinity".
seq_clear(#s{ctx = C}) ->
    barrel_inference_nif:kv_seq_rm(C, 0, 0, -1).
...
%% routes to thinking.
step(#s{ctx = C} = S, Ops) ->
    case barrel_inference_nif:step(C, Ops) of
        {ok, Results} ->
...
sampler_new(#s{ctx = C}, Cfg) when is_map(Cfg) ->
    barrel_inference_nif:sampler_new(C, Cfg).

sampler_free(SamplerRef) ->
    barrel_inference_nif:sampler_free(SamplerRef).

apply_chat_template(#s{model = M}, Request) ->
    barrel_inference_nif:apply_chat_template(M, Request).

embed(#s{ctx = C}, Tokens) ->
    barrel_inference_nif:embed(C, Tokens).

set_grammar(#s{ctx = C} = S, Grammar) when is_binary(Grammar) ->
    case barrel_inference_nif:set_grammar(C, Grammar) of
        ok -> {ok, S};
        {error, _} = E -> E
    end;
...
configure_sampler(#s{ctx = C} = S, Cfg) when is_map(Cfg) ->
    case barrel_inference_nif:configure_sampler(C, Cfg) of
        ok -> {ok, S};
        {error, _} = E -> E
    end.

clear_sampler(#s{ctx = C} = S) ->
    ok = barrel_inference_nif:clear_sampler(C),
    {ok, S}.

load_adapter(#s{model = M} = S, Path) ->
    case barrel_inference_nif:adapter_load(M, Path) of
        {ok, AdapterRef} -> {ok, AdapterRef, S};
        {error, _} = E -> E
    end.

unload_adapter(#s{} = S, AdapterRef) ->
    case barrel_inference_nif:adapter_free(AdapterRef) of
        ok -> {ok, S};
        {error, released} -> {ok, S};
        {error, _} = E -> E
    end.
...
apply_adapters(#s{ctx = C} = S, Adapters) ->
    case barrel_inference_nif:set_adapters(C, Adapters) of
        ok -> {ok, S};
        {error, _} = E -> E
    end.
...
->
    ...
    case barrel_inference_nif:forward_with_argmax(Ctx, Input) of
        {ok, Argmax} ->
...
%% Documented in the public barrel_inference:verify/4 doc.
restore_after_verify(Ctx, P, LastPrefixToken) ->
    ...
    _ = barrel_inference_nif:kv_seq_rm(Ctx, 0, P, -1),
    _ = barrel_inference_nif:prefill(Ctx, [LastPrefixToken]),
    ok.
...
thinking_signature(_S, _SeqId, Bytes) when is_binary(Bytes) ->
    case application:get_env(barrel_inference, thinking_signing_key) of
