...
%%
-module(barrel_inference_scheduler).
-moduledoc """
Memory-pressure-driven cache eviction.

Periodically polls a pluggable pressure source (`barrel_inference_pressure`)
and, when the used/total ratio crosses `high_watermark`, asks the
cache to evict slabs until the ratio would drop below
`low_watermark`. The eviction call is `barrel_inference_cache:evict_bytes/2`
...
per-tier quota or via an explicit `barrel_inference_cache:gc/0` call.
Override with `evict_tiers => all` (or a custom list) to include
disk in scheduler-driven eviction.

Disabled by default. Enable via the `barrel_inference` app environment:
...
-record(state, {
    enabled :: boolean(),
    pressure_source :: barrel_inference_pressure:source(),
...
-spec set_pressure_source(barrel_inference_pressure:source()) -> ok.
...
-spec sample() -> barrel_inference_pressure:reading().
sample() ->
...
handle_call(sample, _From, S) ->
    {Used, Total} = barrel_inference_pressure:sample(S#state.pressure_source),
    S1 = S#state{
        last_used = Used,
        last_total = Total,
...
env_config() ->
    case application:get_env(barrel_inference, scheduler) of
...
check_once(#state{pressure_source = Src} = S) ->
    {Used, Total} = barrel_inference_pressure:sample(Src),
    NowNs = monotonic_ns(),
    S1 = S#state{
        last_used = Used,
        last_total = Total,
